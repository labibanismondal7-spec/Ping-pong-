...
async function api(path, method = "GET", body = null) {
  try {
    ...
    toast("Network connection failed — please try again", true);
...
async function apiUpload(path, formData) {
  ...
  const timeout = setTimeout(() => controller.abort(), 120000); // mobile/Cloudflare uploads may take longer than normal API calls
  try {
    const res = await fetch(API + path, {
      method: "POST",
      body: formData,
    ...
    if (!res.ok) {
      return data && typeof data === "object"
        ? data
        : { success: false, message: `Upload failed (${res.status})` };
    }
    return data && typeof data === "object"
      ? data
    ...
    toast(msg, true);
    return { success: false, networkError: true, message: msg };
  } finally {
...
function applySidebarPermissions() {
  ...
  document.querySelectorAll(".side-item[data-permission]").forEach((btn) => {
    ...
    const allowed = visible.has(section) || (section === "chest-frames" && visible.has("chest"));
...
document.querySelectorAll(".side-item").forEach((btn) => {
  btn.addEventListener("click", () => {
    ...
    if (section === "official-inquiry") loadOfficialInquirySection();
    if (section === "merchants") loadMerchants();
    if (section === "diamond-sellers") loadDiamondSellers();
    if (section === "diamond-centers") loadDiamondCenters();
    if (section === "payment-recharge") loadPaymentRecharge();
    if (section === "chest") loadChestLevels();
    if (section === "chest-frames") loadChestFrameLevels();
    if (section === "rooms") loadRoomUiConfig();
    ...
    if (section === "host-salary") hsEnterSection();
    if (section === "agency-commission") acmEnterSection();
    if (section === "kyc-review") kycEnterSection();
    if (section === "withdrawals") wdEnterSection();
...
function renderUsersTable() {
  ...
  filtered.forEach((u) => {
    ...
    tr.innerHTML = `
      ...
      <td>${countryCell}</td>
      <td class="mono"><img src="/images/icons/icon-diamond.png" class="currency-icon" alt="diamond"> ${Number(u.diamonds || 0).toLocaleString()}</td>
      <td class="mono">🫘 ${Number(u.beans || 0).toLocaleString()}</td>
      ...
      <td class="cell-actions">
        ...
        <button class="btn btn-sm btn-warn act-diamonds">Diamonds</button>
        <button class="btn btn-sm btn-ghost act-beans-remove">Remove Beans</button>
    ...
    tr.querySelector(".act-diamonds").addEventListener("click", async () => {
      const val = prompt("New Diamond amount:", u.diamonds);
      if (val === null) return;
      const Diamonds = Number(val);
      if (isNaN(Diamonds) || Diamonds < 0) { toast("Provide a valid number", true); return; }
      const r = await api(`/api/admin/users/${u.mobile}/diamonds`, "POST", { diamonds: Diamonds });
      if (r.success) { toast("Diamonds updated"); loadUsers(); } else toast(r.message, true);
    });
    tr.querySelector(".act-beans-remove").addEventListener("click", async () => {
      const current = Number(u.beans || 0);
      if (current <= 0) { toast("This user has no Beans to remove", true); return; }
      const val = prompt(`Beans to remove from ${u.name} (available: ${current}):`, "");
      if (val === null) return;
      const amount = Math.floor(Number(val));
      if (!Number.isSafeInteger(amount) || amount <= 0 || amount > current) {
        toast("Enter a valid Beans amount within the user's balance", true);
        return;
      }
      const r = await api(`/api/admin/users/${u.mobile}/beans/remove`, "POST", { amount });
      if (r.success) { toast(`Removed ${amount.toLocaleString()} Beans`); loadUsers(); }
      else toast(r.message || "Beans removal failed", true);
    });
...
// ===========================================================================
// ROOM UI SIZE CONTROLS
// ===========================================================================
async function loadRoomUiConfig() {
  const r = await api("/api/admin/room-ui/config");
  if (!r.success || !r.config) return;
  $("room-ui-seat-size").value = r.config.seatSize;
  $("room-ui-frame-scale").value = r.config.frameScale;
  $("room-ui-status").textContent = `Live: seat ${r.config.seatSize}px · frame ${Number(r.config.frameScale).toFixed(2)}×`;
}
$("btn-save-room-ui")?.addEventListener("click", async () => {
  const seatSize = Number($("room-ui-seat-size").value);
  const frameScale = Number($("room-ui-frame-scale").value);
  if (!Number.isFinite(seatSize) || seatSize < 0 || seatSize > 200 || !Number.isFinite(frameScale) || frameScale < 0 || frameScale > 3) {
    toast("Seat must be 0–200px and frame scale 0.00–3.00", true); return;
  }
  const r = await api("/api/admin/room-ui/config", "POST", { seatSize, frameScale });
  if (r.success) {
    $("room-ui-seat-size").value = r.config.seatSize;
    $("room-ui-frame-scale").value = r.config.frameScale;
    $("room-ui-status").textContent = `Saved live: seat ${r.config.seatSize}px · frame ${Number(r.config.frameScale).toFixed(2)}×`;
    toast("Room seat/frame size updated live");
  } else toast(r.message || "Could not save room UI size", true);
});
...
async function loadFrames() {
  ...
  if (r.success) r.frames.forEach((f) => {
    ...
    const row = document.createElement("div");
    row.className = "data-row";
    row.innerHTML = `${f.imageUrl ? `<img src="${escapeHtml(f.imageUrl)}" alt="" style="width:52px;height:52px;object-fit:contain;border-radius:12px;border:1px solid var(--line);background:transparent;margin-right:12px;" loading="lazy" onerror="this.style.display='none'">` : ""}<div class="data-row-main"><b>${escapeHtml(f.name)}</b><span class="sub">${escapeHtml(f.id)}${f.imageUrl ? " · uploaded " + (String(f.imageUrl).toLowerCase().includes(".gif") ? "GIF" : "image") : ""}${f.source === "treasure-chest" ? ` · Chest L${Number(f.chestLevel||0)} Top ${Number(f.chestRank||0)}` : ""}</span></div>${f.vipOnly ? '<span class="badge badge-warn">VIP only</span>' : ""}<button class="btn btn-danger btn-sm frame-delete" data-id="${escapeHtml(f.id)}">Remove</button>`;
    row.querySelector(".frame-delete")?.addEventListener("click", async () => {
      if (!confirm(`Remove frame "${f.name}" from the global Frame Store?`)) return;
      const rr = await api(`/api/admin/frames/${encodeURIComponent(f.id)}`, "DELETE");
      if (rr.success) { toast("Frame removed"); loadFrames(); } else toast(rr.message || "Frame remove failed", true);
    });
    wrap.appendChild(row);
  });
}
...
$("btn-upload-frame").addEventListener("click", async () => {
  ...
  if (!file) { toast("Select a PNG or GIF file", true); return; }
...
// Dedicated Active Agency badge — intentionally separate from generic
// coloured tags so Admin can add/remove this one Agency identity badge
// without overwriting VIP/custom tags.
$("btn-send-agency-badge")?.addEventListener("click", async () => {
  const targetUserId = $("agency-badge-target").value.trim();
  if (!targetUserId) { toast("Provide a User ID", true); return; }
  const r = await api("/api/admin/agency-badge/assign", "POST", { targetUserId });
  if (r.success) { toast(`Agency Tag sent to ${targetUserId}`); $("agency-badge-target").value = ""; }
  else toast(r.message || "Failed", true);
});
$("btn-remove-agency-badge")?.addEventListener("click", async () => {
  const targetUserId = $("agency-badge-target").value.trim();
  if (!targetUserId) { toast("Provide a User ID", true); return; }
  const r = await api("/api/admin/agency-badge/remove", "POST", { targetUserId });
  if (r.success) { toast(`Agency Tag removed from ${targetUserId}`); $("agency-badge-target").value = ""; }
  else toast(r.message || "Failed", true);
});
...
// ===========================================================================
// PROFILE NAME COLOR — single Premium Gradient production style
// ===========================================================================
const namefxTarget = $("namefx-target");
const namefxPreview = $("namefx-preview");
function updateNameFxPreview() {
  if (!namefxPreview) return;
  const value = namefxTarget && namefxTarget.value.trim();
  namefxPreview.textContent = value || "Your Name";
}
if (namefxTarget) namefxTarget.addEventListener("input", updateNameFxPreview);
updateNameFxPreview();

$("btn-namefx-apply").addEventListener("click", async () => {
  const target = namefxTarget.value.trim();
  if (!target) { toast("Enter a username or User ID", true); return; }
  const r = await api("/api/admin/name-effects/assign", "POST", { target });
  if (r.success) toast("Premium name color sent");
  else toast(r.message || "Failed to send name color", true);
});
$("btn-namefx-remove").addEventListener("click", async () => {
  const target = namefxTarget.value.trim();
  if (!target) { toast("Enter a username or User ID", true); return; }
  const r = await api("/api/admin/name-effects/remove", "POST", { target });
  if (r.success) toast("Profile name color removed");
  else toast(r.message || "Failed to remove name color", true);
});
...
$("gift-catalog-list").addEventListener("click", async (e) => {
  ...
  if (editBtn) {
    ...
    const priceStr = prompt("Diamond Price (leave empty to keep unchanged):", "");
...
$("btn-upload-gift").addEventListener("click", async () => {
  ...
  if (!price || price <= 0) { toast("Provide a valid Diamond Price", true); return; }
  const fd = new FormData();
  fd.append("name", name);
...
$("btn-upload-vgift").addEventListener("click", async () => {
  ...
  if (!price || price < 100000) { toast("Diamond Price must be at least 100000", true); return; }
...
// AGENCIES
// ===========================================================================
let agencyControlCache = [];

async function loadAgencies() {
  const previousAgency = $("agency-control-select")?.value || "";
  const r = await api("/api/admin/agency/list");
  const wrap = $("agencies-list");
  if (wrap) wrap.innerHTML = "";
  agencyControlCache = r.success ? (r.agencies || []) : [];
  const select = $("agency-control-select");
  if (select) {
    select.innerHTML = agencyControlCache.length
      ? `<option value="">Select Agency</option>` + agencyControlCache.map(a => `<option value="${escapeHtml(a.agencyId)}">${escapeHtml(a.name)} · ${escapeHtml(a.agencyId)}</option>`).join("")
      : `<option value="">No agencies found</option>`;
    if (previousAgency && agencyControlCache.some(a => a.agencyId === previousAgency)) select.value = previousAgency;
  }
  if (r.success && wrap) r.agencies.forEach((a) => {
    const row = document.createElement("div");
    row.className = "data-row";
    row.innerHTML = `<div class="data-row-main"><b>${escapeHtml(a.name)}</b><span class="sub">ID: ${escapeHtml(a.agencyId)} · Owner: ${escapeHtml(a.ownerUserId)} · Hosts: ${a.hostIds.length} · Rate: ${(a.commissionRate * 100).toFixed(0)}% · Official: ${escapeHtml(a.officialName || "Not assigned")}</span></div><span class="badge badge-ok"><img src="/images/icons/icon-diamond.png" class="currency-icon" alt="diamond"> ${a.earnedDiamonds || 0}</span>`;
    wrap.appendChild(row);
  });
  const selected = select && select.value;
  await loadAgencyOfficials();
  if (selected) await loadAgencyControl(selected);
}

async function loadAgencyControl(agencyId) {
  const meta = $("agency-control-meta"), hosts = $("agency-control-hosts"), activity = $("agency-control-activity");
  if (!agencyId) {
    if (meta) meta.textContent = "Choose an Agency to inspect it.";
    if (hosts) hosts.innerHTML = `<div class="hint">Select an Agency.</div>`;
    if (activity) activity.innerHTML = `<div class="hint">Select an Agency.</div>`;
    $("agency-control-host-count").textContent = "0";
    return;
  }
  if (hosts) hosts.innerHTML = `<div class="hint">Loading live Agency data…</div>`;
  if (activity) activity.innerHTML = `<div class="hint">Loading audit history…</div>`;
  const r = await api(`/api/admin/agency/${encodeURIComponent(agencyId)}/control`);
  if (!r.success) {
    toast(r.message || "Agency control data unavailable", true);
    return;
  }
  const a = r.agency;
  if (meta) meta.innerHTML = `<strong>${escapeHtml(a.name)}</strong><span>Owner: ${escapeHtml(a.ownerUserId)}</span><span>Country: ${escapeHtml(a.countryId)}</span><span>Target: ${Number(a.weeklyTargetValue||0).toLocaleString()} 💎</span><span>Official: ${escapeHtml(a.officialName || "Not assigned")}</span>`;
  $("agency-control-host-count").textContent = String(a.hostCount || 0);
  $("agency-control-official-id").value = a.officialUserId || "";
  $("agency-control-official-meta").innerHTML = a.officialUserId
    ? `<span class="agency-official-avatar">✓</span><div><b>${escapeHtml(a.officialName || a.officialUserId)}</b><small>ID: ${escapeHtml(a.officialUserId)} · Assigned by ${escapeHtml(a.officialAssignedByAdminUsername || "—")}</small></div>`
    : `<span class="agency-official-avatar">◎</span><div><b>Not assigned</b><small>No Official Handler is linked to this Agency yet.</small></div>`;
  $("agency-control-target").value = Number(a.weeklyTargetValue || 0);
  $("agency-control-target-status").textContent = `Current active weekly target: ${Number(a.weeklyTargetValue||0).toLocaleString()} Diamonds`;
  if (hosts) hosts.innerHTML = r.hosts.length ? r.hosts.map(h => `
    <div class="agency-admin-host-row">
      <div class="agency-admin-host-avatar">${h.photo ? `<img src="${escapeHtml(h.photo)}" alt="">` : escapeHtml((h.name||"U")[0])}</div>
      <div class="agency-admin-host-main"><b>${escapeHtml(h.name)}</b><span>ID: ${escapeHtml(h.userId)} · ${h.online ? "Online" : "Offline"} · ${h.isHost ? "Host" : "Not marked Host"}</span></div>
      <button class="btn btn-danger btn-sm agency-admin-remove-host" data-host-id="${escapeHtml(h.userId)}">Remove</button>
    </div>`).join("") : `<div class="agency-empty-state">No active hosts in this Agency.</div>`;
  if (hosts) hosts.querySelectorAll(".agency-admin-remove-host").forEach(btn => btn.addEventListener("click", async () => {
    const hostId = btn.dataset.hostId;
    if (!confirm(`Remove Host ${hostId} from this Agency? The Agency itself will remain intact.`)) return;
    const rr = await api(`/api/admin/agency/${encodeURIComponent(agencyId)}/remove-host`, "POST", { hostUserId: hostId });
    if (rr.success) { toast("Host removed; Agency preserved"); await loadAgencies(); if ($("agency-control-select")) $("agency-control-select").value = agencyId; await loadAgencyControl(agencyId); }
    else toast(rr.message || "Host removal failed", true);
  }));
  if (activity) activity.innerHTML = r.activity.length ? r.activity.map(l => {
    const ok = l.result !== "failed";
    const when = new Date(l.timestamp).toLocaleString();
    const actor = l.adminUsername || "system";
    const target = l.targetId || l.targetType || "—";
    return `<div class="agency-activity-row"><div class="agency-activity-dot ${ok ? "ok" : "bad"}"></div><div><b>${escapeHtml(l.action || "activity")}</b><span>${escapeHtml(actor)} · ${escapeHtml(when)} · ${escapeHtml(target)}</span>${l.failureReason ? `<small>${escapeHtml(l.failureReason)}</small>` : ""}</div><em>${ok ? "SUCCESS" : "FAILED"}</em></div>`;
  }).join("") : `<div class="agency-empty-state">No Agency activity recorded yet.</div>`;
}


async function loadOfficialInquirySection() {
  const select = $("official-inquiry-select"), summary = $("official-inquiry-summary"), body = $("official-inquiry-body");
  if (!select || !summary || !body) return;
  const r = await api("/api/admin/agency-officials");
  if (!r.success) { select.innerHTML = `<option value="">Unavailable</option>`; summary.innerHTML = `<span class="hint">${escapeHtml(r.message || "Official network unavailable")}</span>`; body.innerHTML = `<div class="hint">Official Inquiry could not be loaded.</div>`; return; }
  const officials = r.officials || [];
  select.innerHTML = `<option value="">Select an Official…</option>` + officials.map(o => `<option value="${escapeHtml(o.officialUserId)}">${escapeHtml(o.name || o.officialUserId)} · ${escapeHtml(o.officialUserId)} · ${Number(o.agencyCount||0)} Agencies</option>`).join("");
  summary.innerHTML = `<div><span>Total Officials</span><b>${Number(r.totalOfficials||0).toLocaleString()}</b></div><div><span>Assigned Agencies</span><b>${Number(r.totalAssignedAgencies||0).toLocaleString()}</b></div><div><span>Assigned by Me</span><b>${Number(r.assignedByMe||0).toLocaleString()}</b></div>`;
  if (!officials.length) { body.innerHTML = `<div class="official-empty">No Official accounts found. Assign the Official Tag first.</div>`; return; }
  select.onchange = () => select.value ? renderOfficialInquiry(select.value) : (body.innerHTML = `<div class="hint">Select an Official.</div>`);
  await renderOfficialInquiry(officials[0].officialUserId);
  select.value = officials[0].officialUserId;
}

async function renderOfficialInquiry(officialUserId) {
  const body = $("official-inquiry-body"); if (!body) return;
  body.innerHTML = `<div class="hint">Loading complete Agency inquiry…</div>`;
  const r = await api(`/api/admin/agency-officials/${encodeURIComponent(officialUserId)}/inquiry`);
  if (!r.success) { body.innerHTML = `<div class="hint">${escapeHtml(r.message || "Inquiry unavailable")}</div>`; return; }
  const counts = Object.entries(r.actionCounts || {}).map(([k,v]) => `<span class="agency-inquiry-chip">${escapeHtml(k)} <b>${v}</b></span>`).join("");
  const agencies = (r.agencies || []).map(a => `<div class="official-agency-card"><b>${escapeHtml(a.name || a.agencyId || "Agency")}</b><span>ID: ${escapeHtml(a.agencyId || "—")}</span><small>Owner: ${escapeHtml(a.ownerId || "—")} · Country: ${escapeHtml(a.countryId || "OTHERS")}</small></div>`).join("");
  body.innerHTML = `<div class="official-inquiry-profile"><div class="agency-official-avatar">${escapeHtml((r.official.name||"O").charAt(0).toUpperCase())}</div><div><h3>${escapeHtml(r.official.name || r.official.officialUserId)}</h3><span>ID: ${escapeHtml(r.official.officialUserId)} · Country: ${escapeHtml(r.official.countryId || "OTHERS")}</span><small>Assigned by: ${escapeHtml(r.official.assignedByAdminUsername || "—")}</small></div></div>
    <div class="agency-inquiry-stats"><div><span>Agencies</span><b>${(r.agencies||[]).length}</b></div><div><span>Recorded Actions</span><b>${(r.activity||[]).length}</b></div><div><span>Action Types</span><b>${Object.keys(r.actionCounts||{}).length}</b></div></div>
    <h3 class="sub-head">Linked Agencies</h3><div class="official-agency-grid">${agencies || `<div class="hint">No Agencies assigned to this Official.</div>`}</div>
    <h3 class="sub-head">Activity Summary</h3><div class="agency-inquiry-chips">${counts || `<span class="hint">No Agency activity recorded yet.</span>`}</div>
    <h3 class="sub-head">Audit Timeline</h3><div class="agency-activity-list agency-inquiry-list">${(r.activity||[]).length ? r.activity.map(l => `<div class="agency-activity-row"><div class="agency-activity-dot ${l.result === "failed" ? "bad" : "ok"}"></div><div><b>${escapeHtml(l.action || "activity")}</b><span>${escapeHtml(l.adminUsername || "system")} · ${escapeHtml(l.targetId || "—")}</span><small>${escapeHtml(new Date(l.timestamp).toLocaleString())}${l.failureReason ? " · " + escapeHtml(l.failureReason) : ""}</small></div><em>${l.result === "failed" ? "FAILED" : "SUCCESS"}</em></div>`).join("") : `<div class="hint">No recorded Agency activity for this Official.</div>`}</div>`;
}

const officialInquiryRefresh = $("btn-official-inquiry-refresh");
if (officialInquiryRefresh) officialInquiryRefresh.addEventListener("click", loadOfficialInquirySection);

async function loadAgencyOfficials() {
  const stats = $("agency-official-stats"), list = $("agency-official-list");
  if (!stats || !list) return;
  const r = await api("/api/admin/agency-officials");
  if (!r.success) { stats.innerHTML = ""; list.innerHTML = `<div class="hint">Official network unavailable.</div>`; return; }
  stats.innerHTML = `
    <div><span>Total Officials</span><b>${Number(r.totalOfficials||0).toLocaleString()}</b></div>
    <div><span>Assigned Agencies</span><b>${Number(r.totalAssignedAgencies||0).toLocaleString()}</b></div>
    <div><span>Assigned by Me</span><b>${Number(r.assignedByMe||0).toLocaleString()}</b></div>`;
  list.innerHTML = (r.officials || []).length ? r.officials.map(o => `
    <div class="agency-official-row">
      <div class="agency-official-avatar">${escapeHtml((o.name||"O").charAt(0).toUpperCase())}</div>
      <div class="agency-official-main"><b>${escapeHtml(o.name || o.officialUserId)}</b><span>ID: ${escapeHtml(o.officialUserId)} · ${escapeHtml(o.countryId || "OTHERS")}</span><small>${Number(o.agencyCount||0)} Agency(ies) · Assigned by ${escapeHtml(o.assignedByAdminUsername || "—")}</small></div>
      <button class="btn btn-ghost btn-sm agency-official-inquiry-btn" data-official-id="${escapeHtml(o.officialUserId)}">Inquiry</button>
    </div>`).join("") : `<div class="agency-empty-state">No Official accounts found. Assign the Official Tag first.</div>`;
  list.querySelectorAll(".agency-official-inquiry-btn").forEach(btn => btn.addEventListener("click", () => openAgencyOfficialInquiry(btn.dataset.officialId)));
}

async function openAgencyOfficialInquiry(officialUserId) {
  const box = $("agency-official-inquiry");
  if (!box) return;
  box.classList.remove("hidden");
  box.innerHTML = `<div class="agency-control-card-head"><h4>Official Inquiry</h4><button class="btn btn-ghost btn-sm" id="agency-official-inquiry-close">Close</button></div><div class="hint">Loading complete Agency activity…</div>`;
  const r = await api(`/api/admin/agency-officials/${encodeURIComponent(officialUserId)}/inquiry`);
  if (!r.success) { box.innerHTML = `<div class="agency-control-card-head"><h4>Official Inquiry</h4><button class="btn btn-ghost btn-sm" id="agency-official-inquiry-close">Close</button></div><div class="hint">${escapeHtml(r.message || "Inquiry unavailable")}</div>`; $("agency-official-inquiry-close").onclick=()=>box.classList.add("hidden"); return; }
  const counts = Object.entries(r.actionCounts || {}).map(([k,v]) => `<span class="agency-inquiry-chip">${escapeHtml(k)} <b>${v}</b></span>`).join("");
  box.innerHTML = `<div class="agency-control-card-head"><div><h4>Official Inquiry — ${escapeHtml(r.official.name)}</h4><span class="hint">ID: ${escapeHtml(r.official.officialUserId)} · Assigned by ${escapeHtml(r.official.assignedByAdminUsername || "—")}</span></div><button class="btn btn-ghost btn-sm" id="agency-official-inquiry-close">Close</button></div>
    <div class="agency-inquiry-stats"><div><span>Agencies</span><b>${r.agencies.length}</b></div><div><span>Recorded Actions</span><b>${r.activity.length}</b></div><div><span>Country</span><b>${escapeHtml(r.official.countryId || "OTHERS")}</b></div></div>
    <div class="agency-inquiry-chips">${counts || `<span class="hint">No Agency activity recorded yet.</span>`}</div>
    <div class="agency-activity-list agency-inquiry-list">${r.activity.length ? r.activity.map(l => `<div class="agency-activity-row"><div class="agency-activity-dot ${l.result === "failed" ? "bad" : "ok"}"></div><div><b>${escapeHtml(l.action || "activity")}</b><span>${escapeHtml(l.adminUsername || "system")} · ${escapeHtml(l.targetId || "—")}</span><small>${escapeHtml(new Date(l.timestamp).toLocaleString())}${l.failureReason ? " · " + escapeHtml(l.failureReason) : ""}</small></div><em>${l.result === "failed" ? "FAILED" : "SUCCESS"}</em></div>`).join("") : `<div class="hint">No recorded Agency activity for this Official.</div>`}</div>`;
  $("agency-official-inquiry-close").onclick=()=>box.classList.add("hidden");
}

const agencyControlSelect = $("agency-control-select");
if (agencyControlSelect) agencyControlSelect.addEventListener("change", () => loadAgencyControl(agencyControlSelect.value));
const agencyControlRefresh = $("btn-agency-control-refresh");
if (agencyControlRefresh) agencyControlRefresh.addEventListener("click", () => loadAgencies());
const agencyOfficialRefresh = $("btn-agency-official-refresh");
if (agencyOfficialRefresh) agencyOfficialRefresh.addEventListener("click", () => loadAgencies());
const agencyControlOfficial = $("btn-agency-control-official");
if (agencyControlOfficial) agencyControlOfficial.addEventListener("click", async () => {
  const agencyId = $("agency-control-select").value;
  const officialUserId = $("agency-control-official-id").value.trim();
  if (!agencyId) return toast("Select an Agency first", true);
  if (!officialUserId) return toast("Enter an Official User ID", true);
  const r = await api(`/api/admin/agency/${encodeURIComponent(agencyId)}/official`, "PUT", { officialUserId });
  if (r.success) { toast("Official Handler assigned"); await loadAgencies(); $("agency-control-select").value=agencyId; await loadAgencyControl(agencyId); }
  else toast(r.message || "Could not assign Official", true);
});
const agencyControlOfficialClear = $("btn-agency-control-official-clear");
if (agencyControlOfficialClear) agencyControlOfficialClear.addEventListener("click", async () => {
  const agencyId = $("agency-control-select").value;
  if (!agencyId) return toast("Select an Agency first", true);
  if (!confirm("Remove the Official Handler from this Agency?")) return;
  const r = await api(`/api/admin/agency/${encodeURIComponent(agencyId)}/official`, "PUT", { officialUserId: "" });
  if (r.success) { toast("Official Handler cleared"); await loadAgencies(); $("agency-control-select").value=agencyId; await loadAgencyControl(agencyId); }
  else toast(r.message || "Could not clear Official", true);
});

const agencyControlTarget = $("btn-agency-control-target");
if (agencyControlTarget) agencyControlTarget.addEventListener("click", async () => {
  const agencyId = $("agency-control-select").value;
  const weeklyTarget = Number($("agency-control-target").value);
  if (!agencyId) return toast("Select an Agency first", true);
  if (!Number.isSafeInteger(weeklyTarget) || weeklyTarget < 0) return toast("Target must be a non-negative integer", true);
  const r = await api(`/api/admin/agency-commission/agencies/${encodeURIComponent(agencyId)}/target`, "PUT", { weeklyTarget });
  if (r.success) { toast("Agency target saved"); await loadAgencies(); $("agency-control-select").value = agencyId; await loadAgencyControl(agencyId); }
  else toast(r.message || "Could not save Agency target", true);
} );
...
// ===========================================================================
// MANAGE COIN SELLERS (Wallet page "Diamond Seller List")
// ===========================================================================
async function loadDiamondSellers() {
  const wrap = $("cs-list");
  wrap.innerHTML = "";
  const r = await api("/api/admin/diamond-sellers");
  const list = r.success ? r.sellers : [];
  if (!list.length) { wrap.innerHTML = `<p class="hint">No Diamond Sellers added yet.</p>`; return; }
  list.forEach((s, idx) => {
    const row = document.createElement("div");
    row.className = "data-row";
    if (s.missing) {
    ...
    row.querySelector(".cs-remove").addEventListener("click", async () => {
      if (!confirm(`Remove ${s.user_id} from the Diamond Seller List?`)) return;
      const tr = await api(`/api/admin/diamond-sellers/${s.user_id}/remove`, "POST");
      if (tr.success) { toast("Removed"); loadDiamondSellers(); } else toast(tr.message || "An error occurred", true);
    });
    const upBtn = row.querySelector(".cs-up");
    const downBtn = row.querySelector(".cs-down");
    if (upBtn) upBtn.addEventListener("click", () => reorderDiamondSeller(list, idx, idx - 1));
    if (downBtn) downBtn.addEventListener("click", () => reorderDiamondSeller(list, idx, idx + 1));
    wrap.appendChild(row);
  });
}

async function reorderDiamondSeller(list, fromIdx, toIdx) {
  const orderedUserIds = list.map((s) => s.user_id);
  const [moved] = orderedUserIds.splice(fromIdx, 1);
  orderedUserIds.splice(toIdx, 0, moved);
  const tr = await api("/api/admin/diamond-sellers/reorder", "POST", { orderedUserIds });
  if (tr.success) loadDiamondSellers(); else toast(tr.message || "An error occurred", true);
}
...
$("btn-cs-add").addEventListener("click", async () => {
  ...
  const r = await api("/api/admin/diamond-sellers/add", "POST", { userId, whatsappNumber });
  if (r.success) { toast("Seller added"); $("cs-add-userid").value = ""; $("cs-add-whatsapp").value = ""; loadDiamondSellers(); }
  else toast(r.message || "An error occurred", true);
});

// ===========================================================================
// DIAMOND CENTER — Seller/Reseller Management
// ===========================================================================
function dcHasPerm(perm) {
  return !!(myAdminProfile && myAdminProfile.permissions && myAdminProfile.permissions.includes(perm));
}

async function loadDiamondCenters() {
  const wrap = $("dc-list");
  if (!wrap) return;
  wrap.innerHTML = `<p class="hint">Loading Diamond Centers…</p>`;
  const r = await api("/api/admin/diamond-centers");
  if (!r.success) { wrap.innerHTML = `<p class="hint">${escapeHtml(r.message || "Could not load")}</p>`; return; }
  const list = r.centers || [];
  if (!list.length) {
    wrap.innerHTML = `<div class="panel"><p class="hint">No Diamond Centers created yet.</p></div>`;
    return;
  }
  wrap.innerHTML = "";
  list.forEach(c => {
    ...
    const pkgHtml = (c.packages || []).map(p =>
      `<span class="tag">${escapeHtml(p.label || ("₹" + p.priceINR + " • " + Number(p.diamonds).toLocaleString() + " 💎"))}</span>`
    ).join(" ");
    row.innerHTML = `
      <div class="data-row-main">
        <b>💎 ${escapeHtml(c.userName)} <span class="hint">ID: ${escapeHtml(c.userId)}</span></b>
        <span class="sub">${c.enabled ? "🟢 Active" : "⚪ Disabled"} · Center Balance: <strong>${Number(c.balanceDiamonds || 0).toLocaleString()} 💎</strong> · Recharges: ${Number(c.totalRechargeCount || 0).toLocaleString()}</span>
      </div>
      <div class="form-row" style="margin-top:10px;">
        <input class="field dc-bonus" type="number" min="0" max="20" step="0.1" value="${Number(c.bonusPercent || 0)}" placeholder="Bonus %">
        <button class="btn btn-ghost dc-save-settings">Save Bonus</button>
        <input class="field dc-topup" type="number" min="1" placeholder="Top-up Diamonds">
        <button class="btn btn-primary dc-topup-btn">Top Up Center</button>
        <button class="btn btn-ghost dc-toggle">${c.enabled ? "Disable" : "Enable"}</button>
      </div>
      <div style="margin-top:8px;"><span class="hint">Packages:</span> ${pkgHtml || '<span class="hint">None</span>'}</div>
      <div class="form-row" style="margin-top:8px;">
        <input class="field dc-new-price" type="number" min="1" placeholder="New package ₹">
        <input class="field dc-new-diamonds" type="number" min="1" placeholder="Package Diamonds">
        <input class="field dc-new-label" type="text" placeholder="Label (optional)">
        <button class="btn btn-ghost dc-add-package">Add Package</button>
      </div>
    `;
    row.querySelector(".dc-save-settings").onclick = async () => {
      const bonus = Number(row.querySelector(".dc-bonus").value);
      const rr = await api(`/api/admin/diamond-centers/${encodeURIComponent(c.userId)}/settings`, "PUT", { bonusPercent: bonus });
      if (rr.success) { toast("Diamond Center settings saved"); loadDiamondCenters(); } else toast(rr.message || "Save failed", true);
    };
    row.querySelector(".dc-topup-btn").onclick = async () => {
      const diamonds = Number(row.querySelector(".dc-topup").value);
      if (!diamonds || diamonds <= 0) { toast("Enter Center Diamonds to top up", true); return; }
      if (!confirm(`Top up ${Number(diamonds).toLocaleString()} Diamonds to ${c.userId}?`)) return;
      const rr = await api(`/api/admin/diamond-centers/${encodeURIComponent(c.userId)}/topup`, "POST", { diamonds });
      if (rr.success) { toast("Center topped up"); loadDiamondCenters(); } else toast(rr.message || "Top-up failed", true);
    };
    row.querySelector(".dc-toggle").onclick = async () => {
      const rr = await api(`/api/admin/diamond-centers/${encodeURIComponent(c.userId)}/settings`, "PUT", { enabled: !c.enabled });
      if (rr.success) { toast(c.enabled ? "Center disabled" : "Center enabled"); loadDiamondCenters(); } else toast(rr.message || "Update failed", true);
    };
    row.querySelector(".dc-add-package").onclick = async () => {
      const priceINR = Number(row.querySelector(".dc-new-price").value);
      const diamonds = Number(row.querySelector(".dc-new-diamonds").value);
      const label = row.querySelector(".dc-new-label").value.trim();
      if (!priceINR || !diamonds) { toast("Enter package price and Diamonds", true); return; }
      const packages = (c.packages || []).map(p => ({...p}));
      packages.push({ priceINR, diamonds, label, active: true });
      const rr = await api(`/api/admin/diamond-centers/${encodeURIComponent(c.userId)}/settings`, "PUT", { packages });
      if (rr.success) { toast("Package added"); loadDiamondCenters(); } else toast(rr.message || "Could not add package", true);
    };
    wrap.appendChild(row);
  });
}

$("btn-dc-create")?.addEventListener("click", async () => {
  const userId = $("dc-create-userid").value.trim();
  const bonusPercent = Number($("dc-create-bonus").value || 0);
  const initialDiamonds = Number($("dc-create-topup").value || 0);
  if (!userId) { toast("Enter a Seller User ID", true); return; }
  if (bonusPercent < 0 || bonusPercent > 20) { toast("Bonus must be 0–20%", true); return; }
  const r = await api("/api/admin/diamond-centers/create", "POST", { userId, bonusPercent });
  if (!r.success) { toast(r.message || "Could not create Center", true); return; }
  if (initialDiamonds > 0) {
    const rr = await api(`/api/admin/diamond-centers/${encodeURIComponent(userId)}/topup`, "POST", { diamonds: initialDiamonds });
    if (!rr.success) toast("Center created, but initial top-up failed: " + (rr.message || ""), true);
  }
  toast("Diamond Center created");
  $("dc-create-userid").value = ""; $("dc-create-bonus").value = "0"; $("dc-create-topup").value = "";
  loadDiamondCenters();
});
...
async function loadPrPackages() {
  ...
  list.forEach((p) => {
    ...
    tr.innerHTML = `
      ...
      <td>${(p.bonusDiamonds || 0).toLocaleString()}</td>
      <td>${(p.diamonds + (p.bonusDiamonds || 0)).toLocaleString()}</td>
...
$("btn-pr-add-package").addEventListener("click", async () => {
  ...
  const diamonds = Number($("pr-pkg-diamonds").value);
  const bonusDiamonds = Number($("pr-pkg-bonus-diamonds").value) || 0;
  const label = $("pr-pkg-label").value.trim();
  if (!priceINR || priceINR <= 0) { toast("Enter a valid price", true); return; }
  if (!diamonds || diamonds <= 0) { toast("Enter a valid Diamond amount", true); return; }
  const r = await api("/api/admin/recharge-packages", "POST", { priceINR, diamonds, bonusDiamonds, label });
  if (r.success) {
    toast("Package added");
    $("pr-pkg-price").value = ""; $("pr-pkg-diamonds").value = ""; $("pr-pkg-bonus-diamonds").value = ""; $("pr-pkg-label").value = "";
    loadPrPackages();
...
async function loadPrRecords() {
  ...
  r.items.forEach((t) => {
    ...
    tr.innerHTML = `
      ...
      <td>₹${t.priceINR}</td>
      <td>${t.totalDiamonds.toLocaleString()}</td>
    ...
    if (approveBtn) approveBtn.addEventListener("click", async () => {
      if (!confirm(`Approve this recharge and credit ${t.totalDiamonds.toLocaleString()} Diamonds to ${t.userId}?`)) return;
      const r2 = await api(`/api/admin/recharge-records/${t.id}/approve`, "POST");
      if (r2.success) { toast("Approved — Diamonds credited"); loadPrRecords(); } else toast(r2.message || "Could not approve", true);
    });
...
function renderChestLevelsForm() {
  ...
  chestLevelsCache.forEach((lvl, i) => {
    ...
    panel.innerHTML = `
      ...
      <label class="field-label">Reward pool (JSON array, e.g. [{"type":"diamonds","amount":500}])</label>
...
$("btn-add-chest-level").addEventListener("click", () => {
  ...
  chestLevelsCache.push({ level: chestLevelsCache.length + 1, target: lastTarget + 100000, rewardPool: [{ type: "Diamonds", amount: 500 }] });
  renderChestLevelsForm();
});
...
async function loadChestFrameLevels() {
  const select = $("chest-frame-level");
  if (!select) return;
  const r = await api("/api/chest/config");
  if (!r.success) return;
  select.innerHTML = r.levels.map((l) => `<option value="${Number(l.level)}">Chest Level ${Number(l.level)}</option>`).join("");
}
loadChestFrameLevels();

$("btn-upload-chest-frame")?.addEventListener("click", async () => {
  const file = $("chest-frame-file")?.files?.[0];
  const level = Number($("chest-frame-level")?.value || 0);
  const rank = Number($("chest-frame-rank")?.value || 0);
  const name = $("chest-frame-name")?.value.trim();
  if (!file || !level || ![1,2,3].includes(rank)) { toast("Choose Level, Top rank and a PNG/GIF frame", true); return; }
  const fd = new FormData();
  fd.append("frame", file); fd.append("level", String(level)); fd.append("rank", String(rank)); if (name) fd.append("name", name);
  const r = await apiUpload("/api/admin/chest/frame-upload", fd);
  if (r.success) {
    toast(`Top ${rank} chest frame uploaded`);
    $("chest-frame-file").value = ""; $("chest-frame-name").value = "";
    loadChestLevels();
  } else toast(r.message || "Chest frame upload failed", true);
});
...
const COUNTRY_LABELS = { IN:"India", BD:"Bangladesh", PK:"Pakistan", US:"United States", GB:"United Kingdom", AE:"UAE", SA:"Saudi Arabia", NP:"Nepal", LK:"Sri Lanka", MY:"Malaysia", AR:"Arabic", OTHERS:"Others" };
...
// ===========================================================================
// OWNER ACCESS CONTROL — granular + Add / Remove permission editor
// ===========================================================================
// This UI is intentionally a thin client over the real RBAC API. The
// server remains authoritative: only the Owner can reach role:manage, the
// account mutation is permission-gated, role ceilings are enforced by
// rbac.js, and every change is audit-logged.
let ramAccessCatalog = null;
let ramAccessAccounts = [];
let ramAccessAccountId = null;
let ramAccessWorking = [];

const RAM_ROLE_LABELS = ROLE_LABELS;

async function loadAccessControl() {
  const panel = $("ram-access-control-panel");
  if (!panel) return;
  $("ram-ac-error").classList.add("hidden");

  const [catalogRes, accountsRes] = await Promise.all([
    api("/api/admin/access-control/catalog"),
  ...
  ramAccessCatalog = catalogRes.catalog;
  ramAccessAccounts = accountsRes.success ? (accountsRes.accounts || []) : [];

  const accountSel = $("ram-ac-account");
  const previous = ramAccessAccountId;
  accountSel.innerHTML = ramAccessAccounts.map((a) =>
    `<option value="${escapeHtml(a.id)}">${escapeHtml(a.username)} — ${escapeHtml(RAM_ROLE_LABELS[a.role] || a.role)}${a.countryId ? " · " + escapeHtml(COUNTRY_LABELS[a.countryId] || a.countryId) : ""}</option>`
  ).join("");

  const moduleSel = $("ram-ac-module");
  const currentModule = moduleSel.value;
  moduleSel.innerHTML = '<option value="">All Modules</option>' +
    Object.keys(ramAccessCatalog.groups).sort().map((m) =>
      `<option value="${escapeHtml(m)}">${escapeHtml(m)}</option>`
    ).join("");
  moduleSel.value = currentModule;

  if (!ramAccessAccounts.length) {
    ramAccessAccountId = null;
    renderAccessControl();
    return;
  }
  ramAccessAccountId = ramAccessAccounts.some((a) => a.id === previous) ? previous : ramAccessAccounts[0].id;
  accountSel.value = ramAccessAccountId;
  prepareAccessControlAccount();
}

function prepareAccessControlAccount() {
  if (!ramAccessCatalog) return;
  const acc = ramAccessAccounts.find((a) => a.id === ramAccessAccountId);
  if (!acc) {
    ramAccessWorking = [];
    renderAccessControl();
    return;
  }
  // Use server-computed effective permissions, not a client-side role
  // default reconstruction. This correctly handles legacy and custom sets.
  ramAccessWorking = Array.isArray(acc.effectivePermissions)
    ? acc.effectivePermissions.slice()
    : (Array.isArray(acc.permissions) ? acc.permissions.slice() : []);
  renderAccessControl();
}

function ramAccessCanEdit(acc) {
  return !!acc && acc.role !== "owner";
}

function ramAccessFiltered(permissionIds) {
  const search = ($("ram-ac-search").value || "").trim().toLowerCase();
  const module = $("ram-ac-module").value || "";
  return permissionIds.filter((id) => {
    if (module && id.split(":")[0] !== module) return false;
    if (!search) return true;
    const label = (ramAccessCatalog.permissionLabels[id] || "").toLowerCase();
    return id.toLowerCase().includes(search) || label.includes(search);
  });
}
...
function renderAccessControl() {
  if (!ramAccessCatalog) return;
  const acc = ramAccessAccounts.find((a) => a.id === ramAccessAccountId);
  const assigned = Array.from(new Set(ramAccessWorking));
  const assignedSet = new Set(assigned);
  const available = ramAccessCatalog.permissions.filter((p) => !assignedSet.has(p));
  const canEdit = ramAccessCanEdit(acc);

  $("ram-ac-summary").innerHTML = acc
    ? `<b>${escapeHtml(acc.username)}</b> · ${escapeHtml(RAM_ROLE_LABELS[acc.role] || acc.role)} · ` +
      `${acc.countryId ? escapeHtml(COUNTRY_LABELS[acc.countryId] || acc.countryId) : "Global"} · ` +
      `${acc.permissionsExplicit ? "Custom permission set" : "Role default permission set"}`
    : "No account selected";

  const warning = acc?.role === "owner"
    ? '<div class="ram-ac-warning">Owner has implicit full access. Owner permissions cannot be removed because Owner is the protected root role.</div>'
    : (acc && ramAccessCatalog.nonOwnerOnly.some((p) => assignedSet.has(p))
      ? '<div class="ram-ac-warning">System-protected permissions are shown only if legacy data contains them; the backend will strip them from non-owner accounts.</div>'
      : "");
  $("ram-ac-error").innerHTML = warning;
  $("ram-ac-error").classList.toggle("hidden", !warning);

  const renderItems = (ids, action, emptyText) => {
    const filtered = ramAccessFiltered(ids);
    if (!filtered.length) return `<div class="ram-ac-empty">${escapeHtml(emptyText)}</div>`;
    return filtered.map((id) => {
      const label = ramAccessCatalog.permissionLabels[id] || id;
      const icon = action === "remove" ? "−" : "+";
      const disabled = !canEdit ? "disabled" : "";
      return `<div class="ram-ac-item">
        <div class="ram-ac-item-main">
          <div class="ram-ac-item-label">${escapeHtml(label)}</div>
          <div class="ram-ac-item-id">${escapeHtml(id)}</div>
        </div>
        <button class="btn ${action === "remove" ? "btn-danger-sm" : "btn-primary"} btn-sm ram-ac-action"
                data-perm="${escapeHtml(id)}" data-action="${action}" ${disabled}
                title="${action === "remove" ? "Remove permission" : "Add permission"}">${icon}</button>
      </div>`;
    }).join("");
  };

  $("ram-ac-assigned-count").textContent = String(assigned.length);
  $("ram-ac-available-count").textContent = String(available.length);
  $("ram-ac-assigned").innerHTML = renderItems(assigned, "remove", "No permissions assigned.");
  $("ram-ac-available").innerHTML = renderItems(available, "add", "No matching permissions available.");

  document.querySelectorAll(".ram-ac-action").forEach((btn) => {
    btn.addEventListener("click", () => {
      const perm = btn.getAttribute("data-perm");
      const action = btn.getAttribute("data-action");
      ramAccessMutatePermission(perm, action);
    });
  });
}

async function ramAccessMutatePermission(permission, action) {
  const acc = ramAccessAccounts.find((a) => a.id === ramAccessAccountId);
  if (!ramAccessCanEdit(acc)) return;

  const current = new Set(ramAccessWorking);
  if (action === "add") current.add(permission);
  else current.delete(permission);

  const next = Array.from(current);
  // Client-side dependency hint; the backend remains authoritative.
  if (action === "add" && permission.includes(":edit")) {
    const view = permission.replace(":edit", ":view");
    if (ramAccessCatalog.permissions.includes(view)) current.add(view);
  }
  const payloadPermissions = Array.from(current);

  const button = document.querySelector(`.ram-ac-action[data-perm="${CSS.escape(permission)}"][data-action="${action}"]`);
  if (button) button.disabled = true;

  const r = await api(`/api/admin/accounts/${encodeURIComponent(acc.id)}`, "PUT", {
    permissions: payloadPermissions
  });
  if (!r.success) {
    toast(r.message || "Permission change failed", true);
    return;
  }

  const updated = r.account || {};
  ramAccessWorking = Array.isArray(updated.effectivePermissions)
    ? updated.effectivePermissions.slice()
    : payloadPermissions.slice();
  const idx = ramAccessAccounts.findIndex((a) => a.id === acc.id);
  if (idx >= 0) ramAccessAccounts[idx] = Object.assign({}, ramAccessAccounts[idx], updated);
  renderAccessControl();
  toast(action === "add" ? `Permission added: ${permission} ✅` : `Permission removed: ${permission}`);
}

async function ramAccessResetToRoleDefaults() {
  const acc = ramAccessAccounts.find((a) => a.id === ramAccessAccountId);
  if (!ramAccessCanEdit(acc) || !ramAccessCatalog.defaultRolePermissions[acc.role]) return;
  if (!confirm(`Reset ${acc.username} to the default permissions for ${RAM_ROLE_LABELS[acc.role] || acc.role}?`)) return;

  const perms = ramAccessCatalog.defaultRolePermissions[acc.role].slice();
  const r = await api(`/api/admin/accounts/${encodeURIComponent(acc.id)}`, "PUT", { permissions: perms });
  if (!r.success) return toast(r.message || "Could not reset permissions", true);
  const updated = r.account || {};
  ramAccessWorking = Array.isArray(updated.effectivePermissions) ? updated.effectivePermissions.slice() : perms;
  const idx = ramAccessAccounts.findIndex((a) => a.id === acc.id);
  if (idx >= 0) ramAccessAccounts[idx] = Object.assign({}, ramAccessAccounts[idx], updated);
  renderAccessControl();
  toast("Role default permissions restored");
}

if ($("ram-ac-account")) $("ram-ac-account").addEventListener("change", () => {
  ramAccessAccountId = $("ram-ac-account").value;
  prepareAccessControlAccount();
});
if ($("ram-ac-search")) $("ram-ac-search").addEventListener("input", renderAccessControl);
if ($("ram-ac-module")) $("ram-ac-module").addEventListener("change", renderAccessControl);
if ($("ram-ac-refresh")) $("ram-ac-refresh").addEventListener("click", loadAccessControl);
if ($("ram-ac-reset")) $("ram-ac-reset").addEventListener("click", ramAccessResetToRoleDefaults);
...
const AC_MODULES = [
  ...
  { key: "recharge",       label: "Recharge",       basePath: "/api/admin/recharge/requests",        viewPerm: "recharge:view",       submitPerm: "recharge:submit",       reviewPerm: "recharge:review",       approvePerm: "recharge:approve",       hasComment: true,  titleField: (r) => (r.amount != null ? r.amount + " Diamonds" : "—") },
...
function anRenderOverview() {
  ...
  const revRechargeDiamonds = revThisMonth.reduce((s, d) => s + (d.recharge.diamonds || 0), 0);
  ...
  const cards = [
    ...
    { num: `${revCoins} Diamonds / ${revDiamonds} 💎`, lbl: "Revenue Summary (this month)" }
  ];
...
function anRenderRevenue() {
  ...
  anCache.revenueDays.forEach((d) => {
    ...
    buckets[key].recharge += d.recharge.diamonds || 0;
    buckets[key].withdraw += d.withdraw.diamonds || 0;
    buckets[key].diamondSales += d.diamondSales.diamonds || 0;
    buckets[key].diamondCommission += d.diamondCommission.beans || 0;
  });
  ...
  anRenderGroupedBarChart($("an-rev-bar1"), rows, [
    { field: "recharge", label: "Recharge (Diamonds)", color: "#4ADE80" },
  ...
  anRenderGroupedBarChart($("an-rev-bar2"), rows, [
    ...
    { field: "diamondCommission", label: "Commission (Beans)", color: "#F7CE7E" }
  ]);
}
...
$("an-btn-export-csv").addEventListener("click", () => {
  const rows = anCache.revenueDays.map((d) => [d.date, d.recharge.diamonds, d.withdraw.diamonds, d.diamondSales.diamonds, d.diamondCommission.beans]);
  const csv = anCsvFromRows(["Date", "Recharge (Diamonds)", "Withdraw (diamonds)", "Diamond Sales (diamonds)", "Diamond Commission (Beans)"], rows);
...
async function chLoadActiveCalls() {
  ...
  list.innerHTML = r.activeCalls.map((c) => `
    <div class="panel">
      ...
      <span class="hint">Elapsed: ${c.elapsedSec}s · Diamonds charged: ${c.diamondsCharged}</span>
    </div>
  `).join("");
}
...
async function chLoadHistory() {
  ...
  list.innerHTML = r.calls.map((c) => `
    <div class="panel">
      <strong>${c.callType}</strong> — ${c.callerId} → ${c.hostId} — ${c.status}<br>
      <span class="hint">${new Date(c.startTime).toLocaleString()} · ${c.durationSec}s · ${c.diamondsCharged} Diamonds</span>
    </div>
  `).join("");
}
  ...
  list.innerHTML = r.reports.map((rep) => `
    <div class="panel">
      ...
      <span class="hint">
        ...
        Diamonds — Today: ${rep.dailyDiamonds} · This week: ${rep.weeklyDiamonds} · This month: ${rep.monthlyDiamonds}<br>
        Last call: ${rep.lastCall ? new Date(rep.lastCall).toLocaleString() : "—"}
        ${rep.target ? `<br>Target: ${rep.target.progressDiamonds}/${rep.target.targetDiamonds} Diamonds (${rep.target.progressPct}%, ${rep.target.remaining} remaining)` : ""}
      </span>
    </div>
  `).join("");
}
...
async function chLoadRevenue() {
  ...
  byday.innerHTML = days.length
    ? days.map((d) => `<div class="panel" style="display:flex;justify-content:space-between;"><span>${d}</span><span>${r.byDay[d]} Diamonds</span></div>`).join("")
...
async function chLoadRates() {
  ...
  if (!r.success) return;
  $("ch-rate-diamonds").value = r.rates.diamondsPerMinute;
...
$("ch-btn-save-rates").addEventListener("click", async () => {
  const body = {
    diamondsPerMinute: Number($("ch-rate-diamonds").value),
...
$("ch-btn-set-target").addEventListener("click", async () => {
  ...
  const targetDiamonds = Number($("ch-target-diamonds").value);
  const periodDays = Number($("ch-target-days").value) || 7;
  if (!userId || !Number.isFinite(targetDiamonds)) { toast("Enter host User ID and target Diamonds", true); return; }
  const r = await api(`/api/admin/call-hosting/targets/${encodeURIComponent(userId)}`, "PUT", { targetDiamonds, periodDays });
  if (r.success) toast("Target set"); else toast(r.error || "Failed", true);
});

// ===========================================================================
// HOST SALARY / BEANS PAYOUT
// ===========================================================================
let hsLoadedPolicy = null;
let hsLastPreview = null;

function hsDateInputDefaults() {
  const now = new Date();
  const first = new Date(now.getFullYear(), now.getMonth(), 1);
  const iso = (d) => { const x = new Date(d.getTime() - d.getTimezoneOffset() * 60000); return x.toISOString().slice(0, 10); };
  if ($("hs-from") && !$("hs-from").value) $("hs-from").value = iso(first);
  if ($("hs-to") && !$('hs-to').value) $("hs-to").value = iso(now);
}

async function hsEnterSection() {
  hsDateInputDefaults();
  await Promise.all([hsLoadPolicy(), hsLoadAgencies(), hsLoadPayouts()]);
  await hsPreview();
}

async function hsLoadPolicy() {
  const r = await api("/api/admin/host-salary/policy");
  if (!r.success) return;
  hsLoadedPolicy = r.policy;
  $("hs-beans-usd").value = r.policy.beansPerUsd;
  $("hs-exchange-rate").value = Number(r.policy.exchangeRate || 0) * 100;
  $("hs-bd-rate").value = r.policy.countries?.BD?.usdLocalRate ?? 120;
  $("hs-in-rate").value = r.policy.countries?.IN?.usdLocalRate ?? 90;
  $("hs-other-rate").value = r.policy.countries?.DEFAULT?.usdLocalRate ?? 1;
  $("hs-policy-table").innerHTML = `<table class="data-table"><thead><tr><th>Level</th><th>Diamond Target</th><th>Beans Reward</th><th>Reward %</th></tr></thead><tbody>${(r.policy.levels || []).map((l) => `<tr><td>Lv.${l.level}</td><td class="mono">${Number(l.targetDiamonds).toLocaleString()}</td><td class="mono">${Number(l.rewardBeans).toLocaleString()}</td><td>${l.targetDiamonds ? ((l.rewardBeans / l.targetDiamonds) * 100).toFixed(0) : 0}%</td></tr>`).join("")}</tbody></table>`;
}

async function hsLoadAgencies() {
  const r = await api("/api/admin/agency/list");
  const sel = $("hs-agency-id");
  if (!sel || !r.success) return;
  const agencies = r.agencies || [];
  sel.innerHTML = `<option value="">Select Agency</option>` + agencies.map((a) => `<option value="${escapeHtml(a.agencyId)}">${escapeHtml(a.name || a.agencyId)} — ${escapeHtml(a.agencyId)}</option>`).join("");
}

function hsQuery() {
  const params = new URLSearchParams();
  if ($("hs-from").value) params.set("from", $("hs-from").value);
  if ($("hs-to").value) params.set("to", $("hs-to").value);
  if ($("hs-country").value) params.set("country", $("hs-country").value);
  return params.toString();
}

function hsRenderPreview(r) {
  hsLastPreview = r;
  $("hs-summary").innerHTML = `
    <div class="stat-card"><span class="stat-num">${r.eligibleCount}</span><span class="stat-lbl">Eligible Hosts</span></div>
    <div class="stat-card"><span class="stat-num">${Number(r.totalBeans || 0).toLocaleString()}</span><span class="stat-lbl">Beans To Send</span></div>
    <div class="stat-card"><span class="stat-num">${(r.rows || []).filter(x => x.alreadyPaid).length}</span><span class="stat-lbl">Already Paid</span></div>`;
  const list = $("hs-preview-list");
  if (!r.rows?.length) { list.innerHTML = "<p class='hint'>No Agency Hosts found for this period.</p>"; return; }
  list.innerHTML = r.rows.map((x) => `<div class="panel" style="margin-bottom:8px;display:flex;justify-content:space-between;gap:12px;align-items:center;">
    <div><strong>${escapeHtml(x.name)}</strong> <span class="mono">${escapeHtml(x.userId)}</span><br><span class="hint">${escapeHtml(x.agencyName)} · ${escapeHtml(x.country)} · ${Number(x.diamonds).toLocaleString()} Diamonds</span></div>
    <div style="text-align:right;"><strong>${x.eligible ? `Lv.${x.level} · ${Number(x.rewardBeans).toLocaleString()} Beans` : "Not eligible"}</strong><br><span class="hint">${x.alreadyPaid ? "Already paid for this period" : x.eligible ? `${Number(x.targetDiamonds).toLocaleString()} target · ${x.cashEstimate.currency} ${Number(x.cashEstimate.local).toLocaleString()}` : "Target not reached"}</span></div>
  </div>`).join("");
}

async function hsPreview() {
  const r = await api("/api/admin/host-salary/preview?" + hsQuery());
  if (r.success) hsRenderPreview(r); else toast(r.message || "Could not calculate salary", true);
}

async function hsSendAll() {
  if (!hsLastPreview || !hsLastPreview.eligibleCount) { await hsPreview(); }
  if (!hsLastPreview?.eligibleCount) { toast("No eligible hosts to pay", true); return; }
  const total = Number(hsLastPreview.totalBeans || 0).toLocaleString();
  if (!confirm(`Send ${total} Beans to ${hsLastPreview.eligibleCount} eligible hosts for this period? This is one automatic salary run and cannot be repeated for the same host/period.`)) return;
  const body = { from: $("hs-from").value, to: $("hs-to").value, country: $("hs-country").value || null };
  const r = await api("/api/admin/host-salary/send-all", "POST", body);
  if (r.success) { toast(`Salary sent: ${r.sent} host(s)`); await hsLoadPayouts(); await hsPreview(); }
  else toast(r.message || "Salary batch failed", true);
}

async function hsManualHost() {
  const body = { userId: $("hs-manual-host-id").value.trim(), beans: Number($("hs-manual-host-beans").value), reason: $("hs-manual-host-reason").value.trim() };
  if (!body.userId || !Number.isFinite(body.beans) || body.beans <= 0) { toast("Enter Host ID and positive Beans", true); return; }
  if (!confirm(`Send ${body.beans.toLocaleString()} Beans to host ${body.userId}?`)) return;
  const r = await api("/api/admin/host-salary/manual-host", "POST", body);
  if (r.success) { toast("Host Beans sent"); $("hs-manual-host-id").value = ""; $("hs-manual-host-beans").value = ""; $("hs-manual-host-reason").value = ""; hsLoadPayouts(); }
  else toast(r.message || "Could not send Beans", true);
}

async function hsManualAgency() {
  const body = { agencyId: $("hs-agency-id").value, beans: Number($("hs-manual-agency-beans").value), reason: $("hs-manual-agency-reason").value.trim() };
  if (!body.agencyId || !Number.isFinite(body.beans) || body.beans <= 0) { toast("Select Agency and enter positive Beans", true); return; }
  if (!confirm(`Send ${body.beans.toLocaleString()} Beans to the Agency Owner for ${body.agencyId}?`)) return;
  const r = await api("/api/admin/host-salary/manual-agency", "POST", body);
  if (r.success) { toast("Agency Beans sent"); $("hs-manual-agency-beans").value = ""; $("hs-manual-agency-reason").value = ""; hsLoadPayouts(); }
  else toast(r.message || "Could not send Agency Beans", true);
}

async function hsSavePolicy() {
  if (!hsLoadedPolicy) return;
  const p = JSON.parse(JSON.stringify(hsLoadedPolicy));
  p.beansPerUsd = Number($("hs-beans-usd").value);
  p.exchangeRate = Number($("hs-exchange-rate").value) / 100;
  p.countries.BD.usdLocalRate = Number($("hs-bd-rate").value);
  p.countries.IN.usdLocalRate = Number($("hs-in-rate").value);
  p.countries.DEFAULT.usdLocalRate = Number($("hs-other-rate").value);
  if (![p.beansPerUsd, p.exchangeRate, p.countries.BD.usdLocalRate, p.countries.IN.usdLocalRate, p.countries.DEFAULT.usdLocalRate].every(Number.isFinite)) { toast("Policy values must be valid numbers", true); return; }
  const r = await api("/api/admin/host-salary/policy", "PUT", p);
  if (r.success) { hsLoadedPolicy = r.policy; toast("Salary policy saved"); hsLoadPolicy(); hsPreview(); }
  else toast(r.message || "Could not save policy", true);
}

async function hsLoadPayouts() {
  const r = await api("/api/admin/host-salary/payouts?limit=100");
  const list = $("hs-payout-history");
  if (!r.success || !r.payouts?.length) { list.innerHTML = "<p class='hint'>No salary/Beans payouts yet.</p>"; return; }
  list.innerHTML = r.payouts.map((p) => `<div class="panel" style="margin-bottom:8px;display:flex;justify-content:space-between;gap:12px;"><div><strong>${escapeHtml(p.method)}</strong> · ${escapeHtml(p.userId || p.agencyId || "—")}<br><span class="hint">${escapeHtml(p.reason || `Level ${p.level || "—"}`)} · ${new Date(p.createdAt).toLocaleString()}</span></div><div style="text-align:right;"><strong>+${Number(p.beans || 0).toLocaleString()} Beans</strong><br><span class="hint">${escapeHtml(p.status)}</span></div></div>`).join("");
}

if ($("hs-btn-preview")) $("hs-btn-preview").addEventListener("click", hsPreview);
if ($("hs-btn-send-all")) $("hs-btn-send-all").addEventListener("click", hsSendAll);
if ($("hs-btn-manual-host")) $("hs-btn-manual-host").addEventListener("click", hsManualHost);
if ($("hs-btn-manual-agency")) $("hs-btn-manual-agency").addEventListener("click", hsManualAgency);
if ($("hs-btn-save-policy")) $("hs-btn-save-policy").addEventListener("click", hsSavePolicy);

// ===========================================================================
// AGENCY WEEKLY TARGET + COMMISSION / SALARY
// ===========================================================================
let acmLoadedPolicy = null;
let acmTiers = [];
let acmHistory = [];

function acmInputDate(iso) {
  const d = new Date(iso); if (Number.isNaN(d.getTime())) return "";
  const p = (n) => String(n).padStart(2, "0");
  return `${d.getFullYear()}-${p(d.getMonth()+1)}-${p(d.getDate())}T${p(d.getHours())}:${p(d.getMinutes())}`;
}
function acmPeriodBody() {
  const start = new Date($("acm-period-start").value);
  const end = new Date($("acm-period-end").value);
  return { periodStart: start.toISOString(), periodEnd: end.toISOString() };
}
function acmRenderPolicy() {
  const wrap = $("acm-policy-rows");
  wrap.innerHTML = acmTiers.map((t, i) => `<div class="panel" style="margin-bottom:8px;display:flex;gap:10px;align-items:end;flex-wrap:wrap;">
    <label class="field-label">Tier <input data-acm-tier-id="${escapeHtml(t.id)}" class="field acm-tier-id" value="${escapeHtml(t.id)}"></label>
    <label class="field-label">Minimum Achieved Target <input type="number" min="0" step="1" class="field acm-tier-min" value="${Number(t.minTargetValue || 0)}"></label>
    <label class="field-label">Commission % <input type="number" min="0" max="100" step="0.01" class="field acm-tier-pct" value="${Number(t.commissionPercent || 0)}"></label>
    <label class="field-label"><input type="checkbox" class="acm-tier-enabled" ${t.enabled !== false ? "checked" : ""}> Enabled</label>
    ${i > 0 ? `<button class="btn btn-danger acm-tier-remove" data-index="${i}">Remove</button>` : ""}
  </div>`).join("");
  wrap.querySelectorAll(".acm-tier-remove").forEach(b => b.addEventListener("click", () => { acmTiers.splice(Number(b.dataset.index), 1); acmRenderPolicy(); }));
}
async function acmLoadPolicy() {
  const r = await api("/api/admin/agency-commission/policy");
  if (!r.success) return;
  acmLoadedPolicy = r.policy; acmTiers = JSON.parse(JSON.stringify(r.policy.tiers || [])); acmRenderPolicy();
}
async function acmSavePolicy() {
  const rows = [...document.querySelectorAll("#acm-policy-rows .panel")];
  const tiers = rows.map(row => ({
    id: row.querySelector(".acm-tier-id").value.trim(),
    minTargetValue: Number(row.querySelector(".acm-tier-min").value),
    commissionPercent: Number(row.querySelector(".acm-tier-pct").value),
    enabled: row.querySelector(".acm-tier-enabled").checked
  }));
  const r = await api("/api/admin/agency-commission/policy", "PUT", { tiers });
  if (r.success) { acmLoadedPolicy = r.policy; acmTiers = JSON.parse(JSON.stringify(r.policy.tiers)); acmRenderPolicy(); toast("Agency commission policy saved"); await acmPreview(); }
  else toast(r.message || "Could not save policy", true);
}
function acmSetPeriod(period) {
  $("acm-period-start").value = acmInputDate(period.start);
  $("acm-period-end").value = acmInputDate(period.end);
}
function acmRenderSummary(summary) {
  const labels = [["Total Agencies",summary.totalAgencies],["Total Target",Number(summary.totalTarget||0).toLocaleString()], ["Total Achieved",Number(summary.totalAchieved||0).toLocaleString()], ["Achieving Target",summary.agenciesAchievingTarget], ["Below Target",summary.agenciesBelowTarget], ["Commission",Number(summary.totalCommission||0).toLocaleString()], ["Paid",Number(summary.totalSalaryPaid||0).toLocaleString()], ["Pending",Number(summary.totalPendingSalary||0).toLocaleString()]];
  $("acm-summary").innerHTML = labels.map(([a,b]) => `<div class="stat-card"><span class="stat-num">${escapeHtml(b)}</span><span class="stat-lbl">${escapeHtml(a)}</span></div>`).join("");
}
function acmRenderRows(rows) {
  const list = $("acm-agency-list");
  if (!rows.length) { list.innerHTML = `<div class="hint">No Agencies in your RBAC scope.</div>`; return; }
  list.innerHTML = rows.map(r => `<div class="panel" style="margin-bottom:8px;display:flex;justify-content:space-between;gap:14px;align-items:center;flex-wrap:wrap;">
    <div><strong>${escapeHtml(r.agencyName)}</strong> <span class="mono">${escapeHtml(r.agencyId)}</span><br><span class="hint">Owner ${escapeHtml(r.agencyOwnerUserId)} · ${escapeHtml(r.periodStart)} → ${escapeHtml(r.periodEnd)}</span></div>
    <div><span class="hint">Target</span> <strong>${Number(r.weeklyTarget||0).toLocaleString()}</strong> · <span class="hint">Achieved</span> <strong>${Number(r.actualAchieved||0).toLocaleString()}</strong><br><span class="hint">${Number(r.achievementPercent||0).toLocaleString()}% · ${escapeHtml(r.commissionTier||"-")} · ${Number(r.commissionPercent||0)}% · Salary ${Number(r.calculatedSalary||0).toLocaleString()} Beans</span></div>
    <div style="text-align:right;min-width:180px;"><strong>${escapeHtml(String(r.status||"pending").toUpperCase())}</strong><br>${r.status === "paid" ? `<button class="btn btn-ghost acm-pdf" data-agency="${escapeHtml(r.agencyId)}">PDF Proof</button>` : `<button class="btn btn-primary acm-pay" data-agency="${escapeHtml(r.agencyId)}">Pay Agency Salary</button>`}</div>
  </div>`).join("");
  list.querySelectorAll(".acm-pay").forEach(b => b.addEventListener("click", () => acmPay(b.dataset.agency)));
  list.querySelectorAll(".acm-pdf").forEach(b => b.addEventListener("click", () => acmPdf(b.dataset.agency)));
}
function acmRenderTargets(rows) {
  const wrap = $("acm-target-list");
  wrap.innerHTML = rows.map(r => `<div class="data-row" style="align-items:center;gap:10px;"><div class="data-row-main"><b>${escapeHtml(r.agencyName)}</b><span class="sub">Owner: ${escapeHtml(r.agencyOwnerUserId)} · Current achieved: ${Number(r.actualAchieved||0).toLocaleString()}</span></div><input type="number" min="0" step="1" class="field acm-target-input" data-agency="${escapeHtml(r.agencyId)}" value="${Number(r.weeklyTarget||0)}" style="max-width:180px;"><button class="btn btn-primary acm-target-save" data-agency="${escapeHtml(r.agencyId)}">Save Target</button></div>`).join("");
  wrap.querySelectorAll(".acm-target-save").forEach(b => b.addEventListener("click", async () => {
    const input = wrap.querySelector(`.acm-target-input[data-agency="${CSS.escape(b.dataset.agency)}"]`);
    const value = Number(input.value);
    if (!Number.isSafeInteger(value) || value < 0) { toast("Target must be a non-negative integer", true); return; }
    const r = await api(`/api/admin/agency-commission/agencies/${encodeURIComponent(b.dataset.agency)}/target`, "PUT", { weeklyTarget: value });
    if (r.success) { toast("Weekly target saved"); await acmPreview(); } else toast(r.message || "Could not save target", true);
  }));
}
async function acmPreview() {
  let body = {};
  try { body = acmPeriodBody(); } catch (_) { toast("Select a valid 7-day period", true); return; }
  const r = await api(`/api/admin/agency-commission/preview?periodStart=${encodeURIComponent(body.periodStart)}&periodEnd=${encodeURIComponent(body.periodEnd)}`);
  if (!r.success) { toast(r.message || "Could not calculate Agency salary", true); return; }
  acmSetPeriod(r.period); acmRenderSummary(r.summary); acmRenderRows(r.rows); acmRenderTargets(r.rows);
}
async function acmPay(agencyId) {
  const body = acmPeriodBody();
  const row = [...document.querySelectorAll(".acm-pay")].find(b => b.dataset.agency === agencyId)?.closest(".panel");
  const text = row ? row.innerText : agencyId;
  if (!confirm(`Confirm Agency salary payment?

${text}

The server will recalculate the tier, salary and wallet credit before payment.`)) return;
  const r = await api("/api/admin/agency-commission/pay", "POST", { agencyId, ...body });
  if (r.success) { toast("Agency salary paid"); await acmPreview(); await acmLoadHistory(); } else toast(r.message || "Payment failed", true);
}
async function acmPayAll() {
  const body = acmPeriodBody();
  if (!confirm("Confirm Pay All Eligible Agencies for this closed 7-day period? Each agency is checked independently and duplicate payouts are rejected.")) return;
  const r = await api("/api/admin/agency-commission/pay-all", "POST", body);
  if (r.success) { toast(`Agency salary: ${r.sent} paid, ${r.failed} failed, ${r.skipped} skipped`); await acmPreview(); await acmLoadHistory(); } else toast(r.message || "Batch payment failed", true);
}
async function acmLoadHistory() {
  const r = await api("/api/admin/agency-commission/history");
  if (!r.success) return;
  acmHistory = r.periods || [];
  const unique = [];
  const seen = new Set();
  for (const p of acmHistory) { const k = `${p.periodStart}|${p.periodEnd}`; if (!seen.has(k)) { seen.add(k); unique.push(p); } }
  $("acm-history-period").innerHTML = `<option value="">Select a previous period</option>` + unique.map(p => `<option value="${escapeHtml(p.periodStart+"|"+p.periodEnd)}">${escapeHtml(p.periodStart)} → ${escapeHtml(p.periodEnd)}</option>`).join("");
  $("acm-history-list").innerHTML = acmHistory.length ? acmHistory.map(p => `<div class="data-row"><div class="data-row-main"><b>${escapeHtml(p.agencyName)}</b><span class="sub">${escapeHtml(p.periodStart)} → ${escapeHtml(p.periodEnd)} · Target ${Number(p.weeklyTarget||0).toLocaleString()} · Achieved ${Number(p.actualAchieved||0).toLocaleString()} · ${Number(p.commissionPercent||0)}%</span></div><div style="text-align:right;"><span class="badge badge-ok">${escapeHtml(String(p.status||"pending").toUpperCase())} · ${Number(p.paidAmount||p.calculatedSalary||0).toLocaleString()} Beans</span>${p.status === "paid" ? `<br><button class="btn btn-ghost acm-history-pdf" data-agency="${escapeHtml(p.agencyId)}" data-start="${escapeHtml(p.periodStart)}" data-end="${escapeHtml(p.periodEnd)}">PDF Proof</button>` : ""}</div></div>`).join("") : `<div class="hint">No saved Agency weekly periods yet.</div>`;
  $("acm-history-list").querySelectorAll(".acm-history-pdf").forEach(b => b.addEventListener("click", () => { $("acm-period-start").value = acmInputDate(b.dataset.start); $("acm-period-end").value = acmInputDate(b.dataset.end); acmPdf(b.dataset.agency); }));
}
async function acmLoadSelectedHistory() {
  const v = $("acm-history-period").value; if (!v) return;
  const [periodStart, periodEnd] = v.split("|");
  $("acm-period-start").value = acmInputDate(periodStart); $("acm-period-end").value = acmInputDate(periodEnd); await acmPreview();
}
async function acmPdf(agencyId) {
  const body = acmPeriodBody();
  try {
    const res = await fetch(API + `/api/admin/agency-commission/pdf?agencyId=${encodeURIComponent(agencyId)}&periodStart=${encodeURIComponent(body.periodStart)}&periodEnd=${encodeURIComponent(body.periodEnd)}`, { headers: token ? {"x-admin-token": token} : {} });
    if (!res.ok) { let m = "Could not generate PDF"; try { m = (await res.json()).message || m; } catch (_) {} toast(m, true); return; }
    const blob = await res.blob(); const url = URL.createObjectURL(blob); const a = document.createElement("a"); a.href = url; a.download = "PINGPONG-AGENCY-SALARY-REPORT.pdf"; document.body.appendChild(a); a.click(); a.remove(); URL.revokeObjectURL(url);
  } catch (_) { toast("PDF download failed", true); }
}
async function acmEnterSection() {
  await acmLoadPolicy();
  const r = await api("/api/admin/agency-commission/agencies");
  if (r.success && r.period) acmSetPeriod(r.period);
  await acmPreview();
  await acmLoadHistory();
}
if ($("acm-btn-add-tier")) $("acm-btn-add-tier").addEventListener("click", () => { const n = acmTiers.length + 1; acmTiers.push({id:`tier-${n}`,minTargetValue:0,commissionPercent:10,enabled:true}); acmRenderPolicy(); });
if ($("acm-btn-save-policy")) $("acm-btn-save-policy").addEventListener("click", acmSavePolicy);
if ($("acm-btn-current")) $("acm-btn-current").addEventListener("click", async () => { const r = await api("/api/admin/agency-commission/agencies"); if (r.success) acmSetPeriod(r.period); await acmPreview(); });
if ($("acm-btn-preview")) $("acm-btn-preview").addEventListener("click", acmPreview);
if ($("acm-btn-pay-all")) $("acm-btn-pay-all").addEventListener("click", acmPayAll);
if ($("acm-btn-load-history")) $("acm-btn-load-history").addEventListener("click", acmLoadSelectedHistory);
...
// ===========================================================================
// WITHDRAWAL KYC (Beans -> USD -> Withdrawal system, 2026-08-31, additive)
// ===========================================================================
let kycSelectedUserId = null;

async function kycEnterSection() { await kycLoadList(); }

function kycStatusColor(s) {
  return s === "VERIFIED" ? "var(--green, #4CAF50)" : s === "REJECTED" ? "var(--red)" : "var(--text-dim)";
}

async function kycLoadList() {
  const status = $("kyc-status-filter").value;
  const r = await api("/api/admin/kyc" + (status ? ("?status=" + encodeURIComponent(status)) : ""));
  const list = $("kyc-list");
  if (!r.success) { list.innerHTML = '<div class="hint">Failed to load.</div>'; return; }
  if (!r.records.length) { list.innerHTML = '<div class="hint">No KYC submissions.</div>'; return; }
  list.innerHTML = r.records.map((k) => `
    <div class="data-row" data-user="${escapeHtml(k.userId)}" style="cursor:pointer;">
      <b>${escapeHtml(k.userId)}</b>
      <span style="color:${kycStatusColor(k.status)};">${escapeHtml(k.status)}</span>
      <span class="hint">PAN: ${escapeHtml(k.panStatus)} · Aadhaar: ${escapeHtml(k.aadhaarStatus)}</span>
      <span class="hint">${escapeHtml(k.submittedAt ? new Date(k.submittedAt).toLocaleString() : "")}</span>
    </div>`).join("");
  list.querySelectorAll("[data-user]").forEach((row) => {
    row.addEventListener("click", () => kycOpenDetail(row.getAttribute("data-user")));
  });
}

async function kycOpenDetail(userId) {
  const r = await api("/api/admin/kyc/" + encodeURIComponent(userId));
  if (!r.success || !r.record) { toast("KYC record not found", true); return; }
  kycSelectedUserId = userId;
  const k = r.record;
  $("kyc-detail-panel").classList.remove("hidden");
  $("kyc-detail-body").innerHTML = `
    <div class="data-row"><b>User</b> ${escapeHtml(k.userId)}</div>
    <div class="data-row"><b>Status</b> <span style="color:${kycStatusColor(k.status)};">${escapeHtml(k.status)}</span></div>
    <div class="data-row"><b>PAN</b> ${escapeHtml(k.pan)} (${escapeHtml(k.panStatus)})</div>
    <div class="data-row"><b>Aadhaar</b> ${escapeHtml(k.aadhaar)} (${escapeHtml(k.aadhaarStatus)})</div>
    <div class="data-row"><b>Account Holder</b> ${escapeHtml(k.accountHolder)}</div>
    <div class="data-row"><b>Account Number</b> ${escapeHtml(k.accountNumber)}</div>
    <div class="data-row"><b>IFSC</b> ${escapeHtml(k.ifsc)}</div>
    <div class="data-row"><b>Bank</b> ${escapeHtml(k.bankName || "—")}</div>
    <div class="data-row"><b>Submitted At</b> ${escapeHtml(k.submittedAt ? new Date(k.submittedAt).toLocaleString() : "—")}</div>
    ${k.rejectionReason ? `<div class="data-row"><b>Last Rejection Reason</b> ${escapeHtml(k.rejectionReason)}</div>` : ""}
  `;
  $("kyc-reject-reason").value = "";
}

$("kyc-btn-refresh").addEventListener("click", kycLoadList);
$("kyc-status-filter").addEventListener("change", kycLoadList);
$("kyc-btn-verify").addEventListener("click", async () => {
  if (!kycSelectedUserId) return;
  const r = await api(`/api/admin/kyc/${encodeURIComponent(kycSelectedUserId)}/verify`, "POST");
  if (r.success) { toast("KYC verified"); await kycLoadList(); $("kyc-detail-panel").classList.add("hidden"); }
  else toast(r.message || "Failed", true);
});
$("kyc-btn-reject").addEventListener("click", async () => {
  if (!kycSelectedUserId) return;
  const reason = $("kyc-reject-reason").value;
  const r = await api(`/api/admin/kyc/${encodeURIComponent(kycSelectedUserId)}/reject`, "POST", { reason });
  if (r.success) { toast("KYC rejected"); await kycLoadList(); $("kyc-detail-panel").classList.add("hidden"); }
  else toast(r.message || "Failed", true);
});

// ===========================================================================
// COUNTRY-AWARE WITHDRAWAL KYC
// ===========================================================================
async function kycEnterSection() {
  const countrySel = $("kyc-admin-country"), statusSel = $("kyc-admin-status");
  if (!countrySel) return;
  if (!countrySel.options.length) {
    const cr = await api("/api/admin/countries");
    const countries = cr.success ? (cr.countries || []) : [];
    countrySel.innerHTML = '<option value="">All Countries</option>' +
      countries.map(c=>`<option value="${escapeHtml(c.id)}">${escapeHtml(c.name)}</option>`).join("");
  }
  await loadKycAdmin();
}
async function loadKycAdmin() {
  const q = new URLSearchParams();
  if ($("kyc-admin-status")?.value) q.set("status",$("kyc-admin-status").value);
  if ($("kyc-admin-country")?.value) q.set("countryId",$("kyc-admin-country").value);
  const r = await api("/api/admin/kyc" + (q.toString() ? "?"+q.toString() : ""));
  const body=$("kyc-admin-tbody"), empty=$("kyc-admin-empty");
  if (!body) return;
  body.innerHTML="";
  if (!r.success || !r.records?.length) { empty?.classList.remove("hidden"); return; }
  empty?.classList.add("hidden");
  r.records.forEach(rec=>{
    const docs=(rec.documents||[]).map(d=>`${escapeHtml(d.key)}:${escapeHtml(d.status)}`).join("<br>") || "—";
    const bank=rec.bankName ? `${escapeHtml(rec.bankName)}<br><span class="mono">${escapeHtml(rec.countryId||"")}</span>` : "—";
    const tr=document.createElement("tr");
    tr.innerHTML=`<td><b>${escapeHtml(rec.userId)}</b></td><td>${escapeHtml(rec.countryName||rec.countryId||"—")}</td><td>${docs}</td><td>${bank}</td><td><span class="badge ${rec.status==="VERIFIED"?"badge-ok":rec.status==="REJECTED"?"badge-danger":"badge-warn"}">${escapeHtml(rec.status)}</span></td><td class="mono">${rec.submittedAt?new Date(rec.submittedAt).toLocaleString():"—"}</td><td class="cell-actions">${rec.status!=="VERIFIED"?`<button class="btn btn-primary btn-sm act-kyc-verify">Verify</button><button class="btn btn-danger btn-sm act-kyc-reject">Reject</button>`:""}</td>`;
    tr.querySelector(".act-kyc-verify")?.addEventListener("click",async()=>{const x=await api(`/api/admin/kyc/${encodeURIComponent(rec.userId)}/verify`,"POST");if(x.success){toast("KYC verified");loadKycAdmin()}else toast(x.message||"Verify failed",true)});
    tr.querySelector(".act-kyc-reject")?.addEventListener("click",async()=>{const reason=prompt("Rejection reason:");if(reason===null)return;const x=await api(`/api/admin/kyc/${encodeURIComponent(rec.userId)}/reject`,"POST",{reason});if(x.success){toast("KYC rejected");loadKycAdmin()}else toast(x.message||"Reject failed",true)});
    body.appendChild(tr);
  });
}
$("kyc-admin-refresh")?.addEventListener("click",loadKycAdmin);
$("kyc-admin-country")?.addEventListener("change",loadKycAdmin);
$("kyc-admin-status")?.addEventListener("change",loadKycAdmin);

// ===========================================================================
// WITHDRAWALS (Beans -> USD -> Withdrawal system, 2026-08-31, additive)
// ===========================================================================
let wdSelectedId = null;

async function wdEnterSection() {
  await wdLoadSettings();
  await wdLoadSummary();
  await wdLoadList();
}

function centsToUsdString(cents) { return "$" + (Math.max(0, Math.floor(cents || 0)) / 100).toFixed(2); }

async function wdLoadSettings() {
  const r = await api("/api/admin/withdrawal-settings");
  if (!r.success) return;
  $("wd-beans-usd").value = r.settings.beansPerUsd;
  $("wd-min-usd").value = (r.settings.minWithdrawalCents / 100).toFixed(2);
  $("wd-enabled").checked = !!r.settings.withdrawalEnabled;
}

$("wd-btn-save-settings").addEventListener("click", async () => {
  const beansPerUsd = Number($("wd-beans-usd").value);
  const minWithdrawalCents = Math.round(Number($("wd-min-usd").value) * 100);
  const withdrawalEnabled = $("wd-enabled").checked;
  const r = await api("/api/admin/withdrawal-settings", "POST", { beansPerUsd, minWithdrawalCents, withdrawalEnabled });
  if (r.success) toast("Withdrawal settings saved");
  else toast(r.message || "Failed", true);
});

async function wdLoadSummary() {
  const r = await api("/api/admin/withdrawals/summary");
  const el = $("wd-summary");
  if (!r.success) { el.innerHTML = ""; return; }
  const s = r.summary;
  const c = s.counts || {};
  const totalRequests = Object.values(c).reduce((sum, n) => sum + (n || 0), 0);
  el.innerHTML = `
    <div class="stat-card"><div class="stat-label">Total Requests</div><div class="stat-value">${totalRequests}</div></div>
    <div class="stat-card"><div class="stat-label">Pending</div><div class="stat-value">${centsToUsdString(s.pendingCents)}</div><div class="hint">${(c.PENDING || 0) + (c.APPROVED || 0)} requests</div></div>
    <div class="stat-card"><div class="stat-label">Processing</div><div class="stat-value">${centsToUsdString(s.processingCents)}</div><div class="hint">${c.PROCESSING || 0} requests</div></div>
    <div class="stat-card"><div class="stat-label">Paid</div><div class="stat-value">${centsToUsdString(s.paidCents)}</div><div class="hint">${c.PAID || 0} requests</div></div>
    <div class="stat-card"><div class="stat-label">Rejected</div><div class="stat-value">${centsToUsdString(s.rejectedCents)}</div><div class="hint">${c.REJECTED || 0} requests</div></div>
  `;
}

function wdStatusColor(s) {
  return s === "PAID" ? "var(--green, #4CAF50)" : (s === "REJECTED" || s === "FAILED") ? "var(--red)" : "var(--text-dim)";
}

async function wdLoadList() {
  const status = $("wd-status-filter").value;
  const userId = $("wd-user-filter").value.trim();
  const fromDate = $("wd-from-filter").value;
  const toDate = $("wd-to-filter").value;
  const params = new URLSearchParams();
  if (status) params.set("status", status);
  if (userId) params.set("userId", userId);
  if (fromDate) params.set("fromDate", fromDate);
  if (toDate) params.set("toDate", toDate);
  const r = await api("/api/admin/withdrawals" + (params.toString() ? ("?" + params.toString()) : ""));
  const list = $("wd-list");
  if (!r.success) { list.innerHTML = '<div class="hint">Failed to load.</div>'; return; }
  if (!r.withdrawals.length) { list.innerHTML = '<div class="hint">No withdrawal requests.</div>'; return; }
  list.innerHTML = r.withdrawals.map((w) => `
    <div class="data-row" data-id="${escapeHtml(w.id)}" style="cursor:pointer;">
      <b>${centsToUsdString(w.amountCents)}</b>
      <span>${escapeHtml(w.userId)}</span>
      <span style="color:${wdStatusColor(w.status)};">${escapeHtml(w.status)}</span>
      <span class="hint">${escapeHtml(new Date(w.createdAt).toLocaleString())}</span>
    </div>`).join("");
  list.querySelectorAll("[data-id]").forEach((row) => {
    row.addEventListener("click", () => wdOpenDetail(row.getAttribute("data-id")));
  });
}

async function wdOpenDetail(id) {
  const r = await api("/api/admin/withdrawals/" + encodeURIComponent(id));
  if (!r.success || !r.withdrawal) { toast("Withdrawal not found", true); return; }
  wdSelectedId = id;
  const w = r.withdrawal;
  $("wd-detail-panel").classList.remove("hidden");
  $("wd-detail-body").innerHTML = `
    <div class="data-row"><b>Withdrawal ID</b> ${escapeHtml(w.id)}</div>
    <div class="data-row"><b>User</b> ${escapeHtml(w.userId)}</div>
    <div class="data-row"><b>Amount</b> ${centsToUsdString(w.amountCents)}</div>
    <div class="data-row"><b>Status</b> <span style="color:${wdStatusColor(w.status)};">${escapeHtml(w.status)}</span></div>
    <div class="data-row"><b>KYC Status (at request time)</b> ${escapeHtml(w.kycSnapshot.status)}</div>
    <div class="data-row"><b>Account Holder</b> ${escapeHtml(w.bankSnapshot.accountHolder)}</div>
    <div class="data-row"><b>Account Number</b> ${escapeHtml(w.bankSnapshot.accountNumber)}</div>
    <div class="data-row"><b>IFSC</b> ${escapeHtml(w.bankSnapshot.ifsc)}</div>
    <div class="data-row"><b>Bank</b> ${escapeHtml(w.bankSnapshot.bankName || "—")}</div>
    <div class="data-row"><b>Requested At</b> ${escapeHtml(new Date(w.createdAt).toLocaleString())}</div>
    ${w.paidAt ? `<div class="data-row"><b>Paid At</b> ${escapeHtml(new Date(w.paidAt).toLocaleString())}</div>` : ""}
    ${w.payoutTxnId ? `<div class="data-row"><b>Payout Reference</b> ${escapeHtml(w.payoutTxnId)}</div>` : ""}
    ${w.rejectionReason ? `<div class="data-row"><b>Reject Reason</b> ${escapeHtml(w.rejectionReason)}</div>` : ""}
  `;
  $("wd-payout-txn").value = "";
  $("wd-reject-reason").value = "";
}

$("wd-btn-refresh").addEventListener("click", wdLoadList);
$("wd-status-filter").addEventListener("change", wdLoadList);
$("wd-from-filter").addEventListener("change", wdLoadList);
$("wd-to-filter").addEventListener("change", wdLoadList);

async function wdDecide(action, extraBody) {
  if (!wdSelectedId) return;
  const r = await api(`/api/admin/withdrawals/${encodeURIComponent(wdSelectedId)}/${action}`, "POST", extraBody || {});
  if (r.success) { toast("Withdrawal updated"); await wdLoadSummary(); await wdLoadList(); await wdOpenDetail(wdSelectedId); }
  else toast(r.message || "Failed", true);
}
$("wd-btn-approve").addEventListener("click", () => wdDecide("approve"));
$("wd-btn-processing").addEventListener("click", () => wdDecide("processing"));
$("wd-btn-paid").addEventListener("click", () => wdDecide("paid", { payoutTxnId: $("wd-payout-txn").value }));
$("wd-btn-reject").addEventListener("click", () => wdDecide("reject", { reason: $("wd-reject-reason").value }));
