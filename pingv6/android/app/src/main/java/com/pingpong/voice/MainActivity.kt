...
class MainActivity : AppCompatActivity() {
    ...
    // ActivityResult launchers are registered from onCreate() so startup
    // initialization follows the Activity lifecycle explicitly. This avoids
    // eager class-level registration and makes the WebView startup path
    // deterministic on Android WebView wrappers.
    private lateinit var filePicker: androidx.activity.result.ActivityResultLauncher<Intent>
    private lateinit var mediaPermissionLauncher: androidx.activity.result.ActivityResultLauncher<Array<String>>
    private lateinit var startupPermissionLauncher: androidx.activity.result.ActivityResultLauncher<Array<String>>
    ...
    private fun registerActivityLaunchers() {
        ...
        mediaPermissionLauncher = registerForActivityResult(
            ...
            if (grantable.isNotEmpty()) request.grant(grantable)
            val denied = request.resources.filter { resource ->
                (resource == PermissionRequest.RESOURCE_AUDIO_CAPTURE && !hasPermission(Manifest.permission.RECORD_AUDIO)) ||
                (resource == PermissionRequest.RESOURCE_VIDEO_CAPTURE && !hasPermission(Manifest.permission.CAMERA))
            }
            if (denied.isNotEmpty()) {
                Toast.makeText(this, "Camera/microphone permission is required for calls and video chat.", Toast.LENGTH_LONG).show()
            }
        }
        ...
        startupPermissionLauncher = registerForActivityResult(
            ...
            if (denied.isNotEmpty()) {
                Toast.makeText(this, "Microphone/camera access is needed for Ping Pong voice rooms. You can enable it in Android settings anytime.", Toast.LENGTH_LONG).show()
            }
        }
    ...
    private fun configureWebView(view: WebView, isMainView: Boolean) {
        ...
        // Keep WebRTC <video> on the WebView's hardware compositor. This is
        // important on Android devices where a software-composited WebView can
        // show a live camera MediaStream as a black surface.
    ...
    private fun handleWebPermissionRequest(request: PermissionRequest) {
        ...
        // WebRTC in WebView asks for AUDIO_CAPTURE and VIDEO_CAPTURE. Filter
        // everything else so future WebView resource types can never be
        // accidentally granted to the web page.
