...
// Atomic cross-currency transfer (e.g. Diamonds -> Beans). Both sides and
// their ledger rows commit or roll back together; this closes the last gap
// where a gift could otherwise require two independent wallet transactions.
async function transferCrossCurrency({ fromUserId, toUserId, fromCurrency, toCurrency, amount, txnId, reason, context }) {
    if (!fromUserId || !toUserId || !fromCurrency || !toCurrency) throw new Error('[module4/wallet] cross transfer identifiers are required');
    assertValidAmount(amount); assertValidTxnId(txnId); const abs=Math.abs(amount); if (abs<=0) throw new Error('[module4/wallet] amount must be > 0');
    const p=db.getPool(); if(!p) throw new Error('[module4/wallet] Postgres not configured');
    const client=await p.connect(); const debitId=`${txnId}:debit`, creditId=`${txnId}:credit`;
    try {
      await client.query('BEGIN');
      const existing=await client.query('SELECT txn_id,status,balance_after FROM module4_wallet_ledger WHERE txn_id IN ($1,$2)',[debitId,creditId]);
      if(existing.rowCount){ await client.query('COMMIT'); const by=Object.fromEntries(existing.rows.map(r=>[r.txn_id,r])); return {txnId,replay:true,debit:by[debitId]||null,credit:by[creditId]||null}; }
      ...
      await client.query(`INSERT INTO module4_wallet_balances(user_id,currency,balance) VALUES($1,$2,0) ON CONFLICT DO NOTHING`,[toUserId,toCurrency]);
      const d=await client.query(`UPDATE module4_wallet_balances SET balance=balance-$1,updated_at=now() WHERE user_id=$2 AND currency=$3 AND balance-$1>=0 RETURNING balance`,[abs,fromUserId,fromCurrency]);
      if(!d.rowCount){await client.query(`INSERT INTO module4_wallet_ledger(txn_id,user_id,currency,amount,status,reason,context,completed_at) VALUES($1,$2,$3,$4,'rejected',$5,$6,now())`,[debitId,fromUserId,fromCurrency,-abs,reason||null,context||null]);await client.query('COMMIT');return {txnId,replay:false,debit:{status:'rejected',reason:'insufficient_funds'},credit:null};}
      const debitBalance=Number(d.rows[0].balance);
      const c=await client.query(`UPDATE module4_wallet_balances SET balance=balance+$1,updated_at=now() WHERE user_id=$2 AND currency=$3 RETURNING balance`,[abs,toUserId,toCurrency]);
      const creditBalance=Number(c.rows[0].balance);
      await client.query(`INSERT INTO module4_wallet_ledger(txn_id,user_id,currency,amount,status,reason,context,balance_after,completed_at) VALUES($1,$2,$3,$4,'completed',$5,$6,$7,now()),($8,$9,$10,$11,'completed',$5,$6,$12,now())`,[debitId,fromUserId,fromCurrency,-abs,reason||null,context||null,debitBalance,creditId,toUserId,toCurrency,abs,creditBalance]);
      await client.query('COMMIT'); return {txnId,replay:false,debit:{status:'completed',balanceAfter:debitBalance},credit:{status:'completed',balanceAfter:creditBalance}};
