...
function makeStore(dataFolder) {
    ...
    const PERMISSIONS = [
        ...
        "rooms:global-control",
        "users:view", "users:edit", "users:mute", "users:ban", "users:unban", "users:delete", "users:diamond-edit", "users:verify",
        ...
        "diamond-seller:view", "diamond-seller:manage",
        "diamond-center:view", "diamond-center:manage", "diamond-center:topup",
        "announce:send",
        "chest:manage",
        ...
        "host-salary:manage", // Owner-only Host Salary / Beans payout engine
        "agency-salary:manage", // Agency Weekly Target + Commission / Salary
        "kyc:view", "kyc:review", // Withdrawal KYC (PAN/Aadhaar/Bank) — review = manually verify or reject a submission
        "payout:view", "payout:review", "payout:approve", // Beans->USD Withdrawal requests — review = approve/reject/mark-processing, approve = mark PAID (final money movement)
        "payout-settings:manage" // Beans/$1 rate + minimum withdrawal + enable/disable — see payoutWithdrawal.js
    ];
    ...
    const NON_OWNER_ONLY = ["country:manage", "role:manage", "security:server-settings", "banners:manage", "host-salary:manage", "payout-settings:manage"];
    ...
    const DEFAULT_ROLE_PERMISSIONS = {
        ...
        [ROLES.COUNTRY_MANAGER]: [
            ...
            "agencies:view", "agencies:manage", "agencies:review", "diamond-seller:view", "diamond-seller:review", "diamond-center:view", "announce:send",
            ...
            "kyc:view", "kyc:review", "payout:view", "payout:review" // can manually verify/reject withdrawal KYC and approve/reject/mark-processing a withdrawal, but not mark PAID (payout:approve) or touch the exchange rate (payout-settings:manage stays Owner-only)
        ],
        [ROLES.ADMIN]: [
            ...
            "vip:view", "vip:submit", "namefx:view", "namefx:submit", "frames:view", "frames:manage", "frames:submit",
            ...
            "kyc:view", "payout:view" // can see the KYC/withdrawal queues but not decide on them (that's Country Manager+)
        ],
    ...
    function clampPermissionsForRole(role, permissions) {
        let list = Array.isArray(permissions) ? permissions.filter((p) => PERMISSIONS.includes(p)) : [];
        if (role === ROLES.MODERATOR) list = list.filter((p) => MODERATOR_MAX_PERMISSIONS.includes(p));
        // Privilege-escalation hard cap: NON_OWNER_ONLY permissions
        // (country:manage, role:manage, security:server-settings,
        // banners:manage) must never end up in any non-owner account's
        // stored `permissions`, no matter what a creator/actor tries to
        // grant. Owner never stores a permissions array (implicit-all, see
        // effectivePermissions), so the only way one of these could "leak"
        // onto another account is through this function — enforced here
        // unconditionally, independent of who's calling.
        if (role !== ROLES.OWNER) list = list.filter((p) => !NON_OWNER_ONLY.includes(p));
        return list;
    }

    // Privilege-escalation prevention (SRS item 18): an actor may only
    // delegate a permission they currently hold themselves — Owner is the
    // sole exception (implicit-all). Applied on top of, not instead of,
    // clampPermissionsForRole's role-cap and the NON_OWNER_ONLY system cap
    // above. Without this, e.g. a Country Manager who lacks "role:manage"
    // couldn't grant it directly (NON_OWNER_ONLY blocks that specific one),
    // but could still hand a Moderator they created every permission THEY
    // don't personally have, which is the broader escalation vector this
    // closes.
    function delegablePermissions(actor, requested) {
        const list = Array.isArray(requested) ? requested : [];
        if (!actor || actor.role === ROLES.OWNER) return list;
        const actorPerms = effectivePermissions(actor);
        return list.filter((p) => actorPerms.includes(p));
    }
    ...
    function effectivePermissions(acc) {
        ...
        if (acc.permissionsExplicit === true) return Array.isArray(acc.permissions) ? acc.permissions.slice() : [];
    ...
    function createAccount({ creator, username, password, fullName, role, countryId, assignedRoomIds, permissions }) {
        ...
        const acc = {
            ...
            // Item 18: creator can only pass down permissions it holds
            // itself (delegablePermissions), and the result is still capped
            // by the target role's ceiling (clampPermissionsForRole) —
            // e.g. NON_OWNER_ONLY permissions like "role:manage" can never
            // be delegated to a non-owner account by anyone, including
            // Owner delegating through this path.
            permissions: clampPermissionsForRole(role, delegablePermissions(creator, permissions)),
            // `permissionsExplicit` distinguishes an intentional empty/custom
            // permission set from legacy accounts that rely on role defaults.
            // This is required so the Access Control UI can remove the final
            // permission without silently restoring the role defaults.
            permissionsExplicit: Array.isArray(permissions),
            status: "active",
    ...
    function updateAccount(actor, id, updates) {
        ...
        // Item 18: an actor can never modify an account whose role is as
        // powerful as, or more powerful than, their own. This one check
        // covers three of the SRS's escalation vectors at once: editing a
        // peer, editing a higher-level admin, AND self-escalation (a
        // self-edit always compares equal rank against itself, so there is
        // no separate "acc.id === actor.id" carve-out needed — nor wanted,
        // since a self-carve-out is exactly how self-escalation would slip
        // back in). Owner is the sole exception. Account-holders change
        // their own password through their own login session elsewhere,
        // not through this admin-management path.
        if (actor.role !== ROLES.OWNER && ROLE_RANK[acc.role] <= ROLE_RANK[actor.role]) {
            return { success: false, message: "Cannot modify an account of equal or higher rank, including your own" };
        }
        ...
        // Item 18: same delegation rule as createAccount — actor can only
        // grant permissions it holds itself, still capped by the target's
        // role ceiling.
        if (Object.prototype.hasOwnProperty.call(updates, "permissions")) {
            ...
            acc.permissions = clampPermissionsForRole(acc.role, delegablePermissions(actor, updates.permissions));
            acc.permissionsExplicit = true;
        }
    ...
    function deleteAccount(actor, id) {
        ...
        // Item 18: same rank guard as updateAccount — can't delete a peer,
        // a higher-level admin, or (equal rank against itself) their own
        // account.
        if (actor.role !== ROLES.OWNER && ROLE_RANK[acc.role] <= ROLE_RANK[actor.role]) {
            return { success: false, message: "Cannot delete an account of equal or higher rank, including your own" };
        }
        if (actor.role !== ROLES.OWNER && actor.role !== ROLES.GLOBAL_SUPER_ADMIN && acc.countryId !== actor.countryId) {
    ...
    function sanitize(acc) {
        ...
        return Object.assign({}, rest, {
            permissions: Array.isArray(acc.permissions) ? acc.permissions.slice() : [],
            effectivePermissions: effectivePermissions(acc),
            permissionsExplicit: acc.role === ROLES.OWNER ? false : acc.permissionsExplicit === true
        });
    }
    ...
    const SECTION_PERMISSIONS = {
        ...
        "diamond-sellers": "diamond-seller:view", // Wallet Diamond Seller List management
        "diamond-centers": "diamond-center:view", // Diamond Center reseller management
        ...
        "host-salary": "host-salary:manage", // Owner-only host salary / beans payout
        "agency-commission": "agency-salary:manage", // Agency Weekly Target / Commission / Salary
        "kyc-review": "kyc:view", // Withdrawal KYC review queue, see payoutWithdrawal.js. kyc:view gates section visibility; kyc:review gates the Verify/Reject actions inside it
        "withdrawals": "payout:view" // Beans->USD Withdrawal admin panel. payout:view gates section visibility; payout:review/payout:approve/payout-settings:manage gate individual actions inside it
    };

    // Access Control catalog exposed to the Owner-only admin UI. Keeping the
    // catalog server-side prevents the panel from inventing permission strings
    // that the authorization layer does not recognize.
    const PERMISSION_LABELS = {
        "dashboard:view": "View Dashboard",
        "admin-accounts:manage": "Create/Manage Subordinate Admin Accounts",
        "rooms:global-control": "Global Room Control (all rooms)",
        "users:view": "View Users", "users:edit": "Edit Users", "users:mute": "Mute Users",
        "users:ban": "Ban Users", "users:unban": "Unban Users", "users:delete": "Delete Users",
        "users:diamond-edit": "Edit User Coins", "users:verify": "Verify Users",
        "rooms:view": "View Rooms", "rooms:lock": "Lock Rooms", "rooms:unlock": "Unlock Rooms",
        "rooms:delete": "Delete Rooms", "rooms:seat-manage": "Manage Seats/Games",
        "rooms:kick-user": "Kick Users", "rooms:mute-user": "Mute Users", "rooms:seat-lock": "Seat/Mic Lock",
        "economy:view": "View Economy",
                "recharge:view": "View Recharge", "recharge:verify": "Verify Recharge", "recharge:submit": "Submit Recharge",
        "recharge:review": "Review Recharge", "recharge:approve": "Approve Recharge",
        "withdraw:view": "View Withdrawals", "withdraw:approve": "Approve Withdrawals",
        "withdraw:submit": "Submit Withdrawals", "withdraw:review": "Review Withdrawals",
        "frames:view": "View Frames", "frames:manage": "Manage Frames", "frames:submit": "Submit Frames",
        "frames:review": "Review Frames", "frames:approve": "Approve Frames",
        "namefx:view": "View Name Effects", "namefx:approve": "Approve Name Effects",
        "namefx:submit": "Submit Name Effects", "namefx:review": "Review Name Effects",
        "tags:manage": "Manage Tags", "svip-tags:manage": "Manage SVIP Tags",
        "gifts:manage": "Manage Gifts", "gifts:view": "View Gifts", "gifts:submit": "Submit Gifts",
        "gifts:review": "Review Gifts", "gifts:approve": "Approve Gifts", "video-gifts:manage": "Manage Video Gifts",
        "agencies:view": "View Agencies", "agencies:submit": "Submit Agencies", "agencies:review": "Review Agencies",
        "agencies:approve": "Approve Agencies", "agencies:manage": "Manage Agencies",
        "diamond-seller:view": "View Diamond Sellers", "diamond-center:view": "View Diamond Centers", "diamond-center:manage": "Create/Manage Diamond Centers", "diamond-center:topup": "Top Up Center Diamonds", "diamond-seller:approve": "Approve Diamond Sellers",
        "diamond-seller:submit": "Submit Diamond Sellers", "diamond-seller:review": "Review Diamond Sellers",
        "diamond-seller:suspend": "Suspend Diamond Sellers",
        "diamond-seller:view": "View Diamond Sellers", "diamond-seller:manage": "Manage Diamond Sellers",
        "announce:send": "Send Announcements", "chest:manage": "Manage Treasure Chest",
        "theme-library:manage": "Manage Theme Library", "events:manage": "Manage Events", "events:execute": "Execute Events",
        "vip:view": "View VIP", "vip:approve": "Approve VIP", "vip:submit": "Submit VIP", "vip:review": "Review VIP",
        "ban:view": "View Bans", "ban:submit": "Submit Bans", "ban:review": "Review Bans",
        "ban:approve": "Approve Bans", "ban:appeal-review": "Review Ban Appeals", "ban:appeal-decide": "Decide Ban Appeals",
        "badges:manage": "Manage Badges", "fraud:manage": "Manage Fraud Controls",
        "device-ban:manage": "Manage Device Bans", "ip-ban:manage": "Manage IP Bans",
        "reports:view": "View Reports", "reports:handle": "Handle Reports",
        "country:manage": "Manage Country Assignment", "role:manage": "Manage Roles & Permissions",
        "security:view-logs": "View Security/Audit Logs", "security:server-settings": "Manage Server Security Settings",
        "revenue:view": "View Revenue", "ai-core:view": "View AI Core", "ai-core:manage": "Manage AI Core",
        "support-tickets:manage": "Manage Support Tickets", "godpower:manage": "Manage Super Admin/God Power",
        "banners:manage": "Manage Home Banners", "vehicles:manage": "Manage Vehicles",
        "level:manage": "Manage Levels", "callhosting:manage": "Manage Call Hosting",
        "voice-sfu:manage": "Manage Voice SFU", "merchant:view": "View Merchants", "merchant:manage": "Manage Merchants",
        "relationships:manage": "Manage Relationship Settings", "payment:manage": "Manage Payment Settings",
        "payment:view": "View Recharge Records", "payment:approve": "Approve/Reject Recharge Records",
        "host-salary:manage": "Manage Host Salary / Beans", "agency-salary:manage": "Manage Agency Target / Commission / Salary",
        "kyc:view": "View Withdrawal KYC Queue", "kyc:review": "Verify/Reject Withdrawal KYC",
        "payout:view": "View Withdrawals", "payout:review": "Approve/Reject/Process Withdrawals", "payout:approve": "Mark Withdrawal Paid",
        "payout-settings:manage": "Manage Withdrawal Settings (Exchange Rate)"
    };

    function accessControlCatalog() {
        const groups = {};
        ...
        return {
            ...
            permissionLabels: Object.assign({}, PERMISSION_LABELS),
            groups,
            roles: Object.assign({}, ROLES),
            roleRank: Object.assign({}, ROLE_RANK),
            defaultRolePermissions: Object.fromEntries(
                Object.entries(DEFAULT_ROLE_PERMISSIONS).map(([role, perms]) => [role, perms.slice()])
            ),
            nonOwnerOnly: NON_OWNER_ONLY.slice(),
            moderatorMaxPermissions: MODERATOR_MAX_PERMISSIONS.slice(),
            sectionPermissions: Object.assign({}, SECTION_PERMISSIONS)
        };
    }

    return {
        ROLES, ROLE_RANK, CAN_CREATE, COUNTRIES, COUNTRY_IDS, PERMISSIONS, DEFAULT_ROLE_PERMISSIONS, SECTION_PERMISSIONS,
        PERMISSION_LABELS, accessControlCatalog,
